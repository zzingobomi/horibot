...
- [framework_dogfood_plan.md](docs/framework_dogfood_plan.md) — **node framework 도입 plan (2026-06-24, 다음 세션 anchor)**. 검토 phase 첫 큰 산출물. **Phase 1 / 2 / A 완료** — `@service` / `@subscriber` + composite host (`attach_handler`) + framework 확장 (signature codec / robot_id inject / wildcard expand / dedup) + JointStateCache 변환 + 호출자 6곳 정리. test 회귀 0. **§7.5 reframe — backend_v2 실험실** (§14) — Phase B/C/3/4/5 보류. backend_v2/ 새 폴더 zero-base 박음. Contract First + Binder = 확정. Node 완전 삭제 + Handler/Cache/Worker/Adapter 4분류 = 가설, 코드로 검증. 운영 X = 리라이트 cost 작음. plan = backend_v2 framework 박음 → 첫 component 검증 → 며칠 4 질문 판정 → YES 3-4개 면 도메인 logic 다 옮겨심음 → backend/ discard. 도메인 logic (캘 BA / IRLS / Ruckig / step DSL / TSDF / ChArUco 등) 자체 자체 재배치, 폐기 X. 1급 계약 = `@service` / `@subscriber` / `@publishes` 3개. `@calls` 폐기 (실행 흐름 stale + 분산 caller graph X). bridge = infrastructure scan 제외. §6 메타 질문 7종 + §13 결정 history 16개 (handler 분리 / dogfood 위치 / FastAPI DI / BaseComponent hypothetical / dual mark / @calls 검증 가능성 / cost-based reflex / 자체 분석 안 박음 / Cache motivation ROS-think / 노드 grouping convention / cost-based 재발 / 운영 X 리라이트) — 새 코드 진입 전 anchor. "backend_v2" / "framework 검증" / "Component 분리" 톤 던지면 §14 진입. "framework 진행" / "@publishes" / "system-docs" 톤은 §14 가 흡수. "BaseComponent" 톤은 §13.8 정정 안내.
