...
내부 [RealsenseDriver](realsense_driver.py) singleton 의 raw SDK wrap 을 그대로
활용. 이 adapter 는 `CameraCapture` Protocol 의 method (clean names) + 기존
camera_node 가 사용하는 legacy method (read / read_aligned / set_cloud_enabled /
depth_scale property) 양쪽 다 제공.

caller migration 은 점진 — 새 코드는 Protocol method (`read_color` /
`read_depth_frame` / `set_depth_enabled`) 사용, 기존 camera_node 는 legacy.

naming 패턴은 motor 도메인의 `DynamixelBackend` (Protocol impl) ↔ `DynamixelDriver`
(raw SDK) 와 동일 — 본 파일 = Protocol impl, [realsense_driver.py](realsense_driver.py)
= raw SDK.
"""
...
from modules.camera.adapters.realsense_driver import RealsenseDriver
...
class RealsenseCapture:
    ...
    def read_color(self) -> ColorFrame | None:
        ...
        if not ok or rgb is None:
            return None
        # RealsenseDriver 가 timestamp 노출 안 함 — frame-by-frame 가능시 추후
