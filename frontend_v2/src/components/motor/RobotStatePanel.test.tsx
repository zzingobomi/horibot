...
describe("RobotStatePanel", () => {
  ...
  it("torque toggle 시 Motor.Service.SET_TORQUE call (enabled 반전)", async () => {
    ...
    const callSpy = vi.spyOn(bridge, "callService").mockImplementation(
      (key) =>
        Promise.resolve({
          success: true,
          message: "",
          data:
            String(key).includes("topology")
              ? ({
                  motors: [{ id: 1, kind: "joint" as const }],
                } as never)
              : ({ ok: true } as never),
        }),
    );
    ...
    act(() => {
      useFrameworkStore.getState().setTopicData(
        `event/motor/${ROBOT_ID}/torque_changed`,
        { robot_id: ROBOT_ID, enabled: true },
      );
    });

    const { findByText } = render(<RobotStatePanel robotId={ROBOT_ID} />);
  ...
  it("Motion.Stream.TCP_STATE 의 timestamp_unix old → stale badge 표시", async () => {
    ...
    act(() => {
      useFrameworkStore.getState().setTopicData(
        `stream/motion/${ROBOT_ID}/tcp_state`,
        {
          robot_id: ROBOT_ID,
          seq: 1,
