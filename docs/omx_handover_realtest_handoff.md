...
> ⚠️⚠️⚠️ **2026-07-29 밤: §T.9 가 최신이다** — 수평 제시 실물 통과, 수취는
> omx 세계모델 43mm 이탈이 근인 → **재제시 보정(look-then-move) 도입** +
> so101 조 가림 수정. 크로스캘은 사용자 결정으로 기각 (실측 유지).
>
...
## §T.9. 2026-07-29 밤 — 4·5차 런 판정 + **재제시 보정(look-then-move) 도입**

> 4차(20:39, `debug/handover/20260729_203919`): 봉이 omx 에서 ~5cm 멀리 놓여
> 집기 계획 전멸 — **배치 문제, 코드 무혐의** (성공 런들 중심 x 0.19~0.21 vs
> 이번 0.237. omx top-down 밴드는 r≈0.20 얇은 띠).
>
> 5차(20:43, `debug/handover/20260729_204308`): **§4.9 수정분 실물 통과** — 먼
> 끝 자연손목 pick / 수평 제시 `-t/elev+0` 채택 / 공중 재검출 성공. **수취
> 계획에서 전멸** (64후보, 자세 IK 실패 54).

### T.9.1 근인 2개 (trace + 이미지 + 사용자 관찰 삼각측량)

1. **omx 세계모델 ↔ 실물 43mm 이탈** — 계획 E (0.062,−0.268,0.20) vs so101
   실측 (0.070,−0.225,0.186) = 축 수직 43mm (코드가 "검출 오염 의심" 경고
   후 그대로 겨냥 → razor-thin 수취 밴드 밖 전멸). 같은 4cm 가 관측 이동
   여유도 먹어 사용자가 근접("닿을랑 말랑") 목격. **출처는 캘로 못 잡는
   성분** — §4.8 의 y 34mm 이탈이 **크로스캘 완료 상태에서도** 있었고,
   사용자 이력상 크로스캘이 줄자 실측보다 부정확 → **크로스캘 기각 (사용자
   결정), base_pose 는 실측치 유지.**
2. **so101 본인 조 가림** — `set_gripper(so101, open)` 이 plan_receive 뒤라
   재검출 시점에 조가 닫혀 시야 정중앙(=봉)을 가림 (`debug/detect/
   20260729_203553/0003` — mask 가 손가락에 물려 조각, 점군 177). 펜 시절
   §2 교훈("닫힌 그리퍼 가림")의 so101 재발.

### T.9.2 수정 1차 (같은 날 21:11 런에서 반증됨 — T.9.4 로 대체)

| # | 수정 | 자리 |
| --- | --- | --- |
| 1 | so101 그리퍼 open 을 **수취 관측 앞**으로 | module.py 시나리오 |
| 2 | ~~재제시 = 같은 자세 단일 평행이동 (omx TCP − offset)~~ → **21:11 실물에서 IK 전멸** (offset 40.3mm 실측은 성공 — §4.8 미지수② 확정 데이터). 이동된 TCP (0.114,−0.297,0.213) 가 omx 접선 다양체 가용 밴드 밖 | (폐기 — T.9.4) |

### T.9.3 21:11 런이 준 실측 + offline probe 로 확정한 기하 (중요 — 박제)

- **실측 오차 p = (−7.6, +37.4, −13.1)mm** (perp, `omx_represent.offset_mm`
  40.3). z 성분은 **마스크 가장자리 depth 번짐 오염** (centroid z 0.187 = 봉
  바닥면보다 아래, 점군 하한 0.167 = 책상 방향 번짐) → **z 는 FK 앵커**가 정답
  ("아는 기하는 앵커" 클래스). 유효 보정 = 수평 (−7.6, +37.4)mm.
- probe (실 URDF·캘, `_receive_probe` 동형): 이 오차 아래에서 **so101 수취
  IK 는 실물 봉 위치 (0.070,−0.225) 에서 전멸** (z 를 고쳐도 0/16) — 재제시
  (봉 이동)는 필수가 맞다. 그런데 **수평 접선족은 재계획해도 교집합이 빈다**
  (omx IK 432 기각 + 수취 88 기각 — **ROI 를 6cm 풀고 E-게이트를 꺼도 동일**,
  ROI 는 무혐의). 생존족 = **hang(매달기)** 후보 2~5개 (E z≈0.155, J5=0).
- **직각 배치에서 hang 은 유효하다**: 관측 IK 23~46/128 + **시선 전부 클리어**
  (probe). 옛 "hang=가림 전멸"은 평행 배치의 함수였다 (§4.8 예측 실증).

### T.9.4 수정 2차 (probe 검증 완료 — 실물 재검증 대기)

| # | 수정 | 자리 |
| --- | --- | --- |
| 1 | **재제시 = 제시 전면 재계획**: 실측 오차를 `world_offset` 으로 plan_omx_present 에 주입 — omx IK/명령은 FK 공간, **so101 쪽 게이트(E-ROI/수취 결합 probe)와 h_world 는 실물(FK+offset)로 평가**. 후보 스윕/게이트 전부 재사용 (단일 이동 재발명 폐기). 수평 전멸 시 hang 폴백이 자연 채택 | steps.py `plan_omx_present(world_offset=)` + `PresentPlan.world_offset` + module.py 루프 (재계획→이동→관측 재계획→재검출, 최대 2회) |
| 2 | **z 는 FK 앵커** — represent_offset 은 수평 성분만, plan_receive/so_refine 의 perp 보정도 z 제외 (depth 번짐 오염 실측 근거) | steps.py |
| 3 | **checker omx 몸체 world offset 반영** — 재계획 시 충돌/시선/벽 판정이 실물 위치 기준 (margin 8mm 실물화, "닿을랑 말랑" 근본 대응). run 시작 시 리셋 | collision.py `set_base_offset_b` + module.py |
| 4 | `_PRESENT_LIMIT` 8→48 (offset 재계획 생존 후보가 prefer ~11cm 밖) + **E-ROI 게이트 slack 5cm** (`_E_ROI_SLACK_M` — ROI 는 percentile 휴리스틱, 진짜 판정은 수취 probe. 사용자 지시) | steps.py |

회귀: 실측 오차(40mm) 재계획 sim 잠금 `test_replan_with_measured_offset_finds_candidate`
(feasibility) / 40mm 이탈→재계획→수렴 시나리오 / 수렴 시 비용 0 / z-앵커 순수
계산 (test_handover.py). fast 466 + handover sim 4 전부 초록.

### T.9.5 22:06 런 (6차) — **체인 최초 완주, 마지막 한 뼘에서 실패** + 수정 3차

trace `debug/handover/20260729_220639` (375s): 수평 제시 → 재검출 offset
**(−7.6,+37.1)mm 실측** → 재계획 **hang 채택** (T.9.3 예측 그대로) → 재배향 →
잔차 39→17mm → 2차 재계획 → **plan_receive 성공 (zpos/spin0)** → refine →
진입 → **close 가 허공** (achieved 1943≈close 1935, load 72).

근인 2개 + 수정 (fast 468 + sim 4 초록, 실물 재검증 대기):

1. **수취가 closed-loop 이 아니었다** — 재검출 점군 52개 한 번 + refine 한 번
   (연속 두 측정이 12.5mm 어긋남 = 산포 cm 급) 믿고 open-loop 진입. →
   **수취 servo** (PnP servo.py 이식): pre(3cm, 카메라-봉 ~12.9cm = PnP 검증
   최적 측정 대역)에서 **lateral ≤5mm 수렴까지** 측정→pre 재정렬 반복
   (`_RECV_SERVO_MAX=3`/`_RECV_SERVO_EPS_M=0.005`), 그 후에만 진입.
2. **재계획 스윕이 분 단위** — (tcp×w) 쌍마다 resolve 1콜 = 왕복 수백 회
   (재계획 1회 ~140s, 총 375s). → **후보당 1콜 배치 + alive-loop**
   (plan_so_observe 와 같은 패턴, 채택 순서 불변). 예상 ~10초 대.
3. 재제시 수렴 조이기: `_REPRESENT_PERP_MIN_M` 15→**10mm** / `_REPRESENT_MAX`
   2→**3** (22:06: 잔차 17mm 로 상한 소진한 채 수취 진입 — 조준 예산 잠식).

### T.9.6 밤 세션 종료 시점 상태 (23:21 마지막 런 — 다음 세션 시작점)

마지막 런 `debug/handover/20260729_232140` (63s — 속도 수정 전부 반영 확인:
hang 재계획 3초):

- ✅ 집기 진짜 물림 (gap 93 / load −291) → 제시 → 재검출 (offset 26.6mm) →
  hang 재제시 (3초, sweet 정렬 작동) → 재검출#2
- ❌ **잔차 25.7mm 가 재제시 후에도 남음** — 그립이 약해/미믹 탓에 omx 가
  움직일 때마다 봉이 조 안에서 돌아 보정이 목표를 쫓아다닌다 (23:13 런과
  동일 패턴). 그 잔차 위치가 so101 수취 밴드 밖 → plan_receive 전멸 (58 IK).
- ⚠ 미믹 desync 는 **정비 불가** (사용자). 파지 판정 advisory 유지.
  "제시 자세 도달" verify 의 gap 10/load −32 는 매핑 밀림 탓 — 신뢰 금지.

**다음 세션 착수 (코드, 실물 전 sim 소진할 것):**

1. **수취 자세를 측정된 봉 축으로** — 재검출 점군 PCA 로 w 실측 → 수취
   orient/plan_receive 가 가정 w 대신 소비 (미믹이 비뚤게 물어도 so101 은
   "보이는 봉"을 잡는다). 위치는 이미 closed-loop, 방위만 가정이 남아 있다.
2. **잔차 25mm 문제의 정석** = 재제시 후 omx **정지 상태**에서 잰 마지막
   재검출로 plan_receive 를 세우되, so101 수취 밴드가 그 위치를 못 덮으면
   **랑데부 선택 자체를 "재제시 후 예상 이동(실측 ~±2cm 랜덤)까지 덮는 여유
   지점"으로** — 또는 receive probe 를 실측 잔차 반경으로 넓혀 검증.
   (_REPRESENT_MAX=1 은 이미 반영 — 재제시 반복은 역효과 실증.)
3. 그립 전류 650 은 motors.yaml 반영/pi_hori3 재부팅 대기 — 봉 회전 완화의
   물리 레버.

### T.9.7 다음 런 절차 (PC backend 재시작 필수 — pi 변경 없음)

1. 봉은 omx 관측점 부근, **중심 x 0.19~0.21** 대역 (4차 교훈).
2. 예상: 재계획 hang 채택(수 초) → 잔차 <10mm 수렴 → 수취 servo 수렴 → 파지.
   trace: `world_offset_mm` / `refine` (tick 별) / `verify_grasp[수취 close 직후]`.
3. 남은 미지수: ① 재배향 스윙 중 봉 흘림/책상 스침 ② hang 수취 E z≈0.155
   실물 (servo 가 lateral 은 흡수, z 는 FK 앵커 가정 — 일관되게 z 로 긁으면
   재검토) ③ M1(omx 카메라 모델) 미반영 — 근접 국면 눈 확인 유지.
