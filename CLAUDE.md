...
| motion | MoveJ/MoveL/Jog + PyBullet IK + TCP state (calibration Mirror 첫 consumer) + PLAN_PATH (RRT-Connect 경로 계획 — home 허브 강등, motion.md §12) |
...
| [motion.md](docs/motion.md) | Move/Servo/Jog/Task 4계층 + 산업 매핑 + jog drift 진단 박제 + URDF visual↔FK mismatch (open) + **§11 IK 대수술** + **§12 RRT-Connect 플래너·home 허브 강등 (2026-07-22 — cuRobo/OMPL 보류 근거 포함)**. |
...
- **작업 방식: sim 소진 + 관측성 안전망 (2026-07-15 확정)** — 산출물은 "sim 초록"이 아니라 **"집(실물)에서 깨지더라도 왜 깨졌는지 즉시 알 수 있는 상태로 보내는 것"**. ① **즉흥 코드 쏟고 "끝났습니다" 금지** — 스크립트로 상황을 최대한 가정하며 *"내가 뭘 놓쳤나, 이건 테스트로 못 잡나"*를 1회성 아니라 **계속** 자문. mock 을 시나리오별로 스크립트화 → **구멍 하나 = 테스트 하나**로 결정적 재현 (예: 파지 판정의 false-positive/헛잡음/잡았다-놓침 각각을 mock stall 시뮬로). ② **그래도 실물엔 구멍이 남는다** (인정) — sim 이 못 주는 "신호↔실물 매핑"이 매번 무는 지점. ③ **그래서 로그+debug 를 의도적 안전망으로 촘촘히** — 실패해도 그 데이터만으로 원인분석·해결이 되게. 임계값 등은 추측 말고 실물 첫 런 데이터로 튜닝(하루 날림 → 1회 보정 전환), 가능하면 본 작업 전 초단막 "특성화 루틴"으로 실물 경계값 먼저 확보. ④ 완료 보고 시 **"sim 으로 증명된 것 vs 실물에서 남는 미지수"를 정직하게 구분**. (관측성 인프라 = 분산 로깅 [docs/logging.md](docs/logging.md) + debug 아티팩트, 검증 방법론 = [docs/dev_reference.md](docs/dev_reference.md). 기원: 2026-07-14 PnP 파지 실패인데 "집었다" 오판 + 실행단계 로그 0줄 = 원인분석 불가 사고)
- **Task 아키텍처 확정 (2026-07-12 골격 + 2026-07-13 전면 개정 + 2026-07-14 훅 DX/프리뷰)** — **TaskRunner = wire 무지 범용 감독기** (실행/취소/pause/step 게이트/예외→실패 — runtime·키·robot·계약 무지, 변화는 훅 on_state/on_trace 로 통지, 안 달면 headless. 훅 배선 = 클래스 바디 `task = TaskRunner()` + `@task.on_state` 데코레이터 — Mirror descriptor 동형, 실행 상태는 TaskRunnerState 로 인스턴스별 격리). **모듈이 전부 소유**: 계약(트리거+노출 결정한 조작판+진행 스트림 — 전부 손 선언), 배선, 진행 발행(훅에 자기 발행 메서드), 트리거(서비스/@subscriber/내부 호출 — start 는 아무나 호출), 시나리오. **실행 전 step 트리 = 정적 프리뷰** (tasks/core/preview.py — 소스만 읽는 구조 인덱싱, 실행/모킹 0, if/loop 는 표시만·못 푼 호출은 `<동적>`). **등록 의식 0**: registry/@task/TaskMetadata/GET /tasks 전부 없음 — task 정보 채널은 계약뿐, robot 바인딩/표시 문구는 frontend 전용 페이지 상수. **호출 표면 = ctx.call 하나** (RobotHandle 없음): robot-scoped 키는 `robot_id=` (참여 명부 검증 — 선언 밖 robot 명령 즉시 에러 = STOP 커버리지 보장), agnostic 은 req 필드 (§2.7, agnostic 에 robot_id= 주면 fail-fast). step = 저자가 `@step(title="집기")` 로 지정한 함수 (중첩 = flat trace+depth, step_once = step-into; **병렬 gather = all-stop 의미론 지원** — 회귀 테스트 잠금). 실패: 기술적 실패는 **서비스가 raise** (accepted in-band 폐기), 부정 결과(검출 0/전멸 -1)는 데이터 — 판정은 step. timeout = contract `declare_service_timeouts`. step 간 데이터 = 시나리오 변수/반환값 (ctx.data 류 blackboard 금지). 새 task = pick_and_place 복제 — [docs/task.md](docs/task.md) §3. **STOP 은 안전 의무** — 모듈 stop→cancel + on_abort 참여 robot 전원 Motion.STOP.
