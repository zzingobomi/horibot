...
07-29 밤 4연속 수취 IK 전멸의 근인 확정 + 재설계 완료 — **정본 =
[task.md](task.md) §4.10** (근인: 봉이 omx 조 안에서 84°/70° 회전(미믹 약파지),
계획축 가정 수취가 전멸. 검출 무혐의 — "검출 오염" 1차 가설 오진 철회).
구현: `measure_bar`(점군 PCA 봉 실측) / `_grasp_orients`(잡기 조건 그리드 —
정확 축정렬 과잉 제약 제거) / 협상 루프(`find_receive_shift`+`omx_nudge`) /
실패 경로 trace 전량. 어제 실 점군 재생 = 치명 장면 2/2 통과 (fast 473 +
feasibility sim 4 초록). 실물 체크리스트 = task.md §4.10 말미.
...
**07-19 밤 — scan 빌드 프로세스 격리 (버벅임 근본 수정)**: build_world on 스윕
중 3D 로봇 버벅임 리포트 → off 대조 실험으로 "Open3D 가 bridge 릴레이를 굶김"
확정 → build_mesh 를 별도 프로세스(BELOW_NORMAL)로 격리, progress/pair 진단
로그는 큐 릴레이 (관측성 무손실). task.md §4.3.1 상세. fast 373 + 격리 e2e
sim 1 (실 subprocess/Open3D) 초록. **실물 확인 대상: build_world on 스윕 중
로봇 스트림 부드러움 + 스윕 시간 (~12s CPU 경쟁 회수 기대).**

**07-19 밤 실물 2건**: ① capture∥detect 병렬화가 pose1 스캔 정합을 깨 월드
부챗살 — **원복** (재도입 조건 = 모터 속도 정지 게이트, task.md §4.3.1. 프론트
Web Worker 파싱은 byte-identical 검증으로 무혐의). ② OMX 흰 모터가 cube 로
score 0.57 (통계 컷 0.45 돌파) → OMX 베이스 집으러 감 — **로봇 베이스 점유
반경(13cm) 구조 제외** 신설 (robots.yaml base_pose 주입, task.md §4.3 표.
회귀 테스트 = 사고 시나리오 그대로).

**steps.py 파일 분리 완료 (07-19 저녁 — 07-17 "미완" 항목 해소)**: 단일
1,850줄 → `steps/` 패키지 6분할 (primitives=공용 바닥층·노브 SSOT / search /
world / plan / pick / place — __init__ 이 파일 지도 + 옛 표면 재수출).
servo_pick 은 `_commit_descend`/`_withdraw_with_fallback` 추출로 ~290줄.
**순수 이동 — 테스트 본문 무변경 372 passed 가 동작 불변의 증거** (노브 패치
경로만 steps.primitives.* 로). ⚠ 배운 것: verify_grasp 2회를 헬퍼로 빼면
preview 정적 트리에서 step 이 사라진다 (breakpoint 표면 소실) — preview 안전
잠금이 실제로 잡아서 인라인 유지, pick.py 에 주석 박제. 같은 날 후속 최적화:
capture∥detect 병렬 (스윕 ~45.6→예상 ~37s) + meshopt 재검토 계측·보류 판정
(task.md §4.3.1).
