...
### §12. 모션 플래너 (RRT-Connect) — "무조건 home 경유" 강등 (2026-07-22 구현 완료, 실물 미검증)

> **동기 (사용자 pain)**: 모든 긴 이동(관측 전환/servo 진입/운반)이 무조건
> home 을 왕복 — 물체가 바로 앞이어도 스윙 낭비 + **쥔 채 최장 스윙**(집기
> 후→home→적치)이 시간·낙하 리스크 둘 다 최악. 근본 원인 = 충돌 인지
> 플래너 부재를 "항상 안전 허브(home) 경유"라는 보수적 단순화로 회피했던 것.
> home 은 단순 안전 자세가 아니라 **랑데부 상태** 로 하중을 받고 있었다 —
> place 를 집기 전에 계획하고(§ 순서 규약) 실행은 한참 뒤인데, "실행 직전
> home 을 찍으면 계획 시점의 `path_from=home` 전제가 되살아난다"가 구조의
> 발명이었다. 그래서 빼려면 call site 삭제가 아니라 **실행 직전 현재-관절
> 재계획**이 정석.

**A. 플래너 선택 — PyBullet 위 RRT-Connect 직접 구현 (OMPL/cuRobo 보류)**
- 결정 축 = "충돌검사가 어디서 도나": 어느 라이브러리를 골라도 충돌검사는
  기존 PyBullet 세계 (self/floor/obstacle — resolve 게이트와 같은 kin)에서
  돌아야 충돌 판단 SSOT 가 한 곳. **OMPL**: 실익 = 트리 관리 C++ 뿐인데 상태
  검증 콜백이 C++→Python 경계를 수천 번 넘어 상쇄 + 신규 의존성 — 플래너
  동물원(PRM*/Informed RRT*)이나 constraint manifold 가 필요해지면 재고.
  **cuRobo**: ms급 반응형 재계획이 셀링포인트 — 환경은 **이미 준비됨** (RTX
  3060 12GB + CUDA 12.6 + WSL2 Ubuntu + Docker 확인, 2026-07-22. 경로 = WSL2
  네이티브 설치 + `.wslconfig` `networkingMode=mirrored` 한 줄로 Zenoh
  multicast 무마찰, torch 는 cuRobo 지원 CUDA 12.x 조합으로 WSL 안 새 env).
  보류 근거: 우리 용도 = 준정적 장면에서 **이동 구간당 1회** 계획 — 수십 ms
  변동성도 팔 이동 수 초에 묻힘. 업그레이드 조건 = "움직이는 장애물에 매
  프레임 재계획"이 필요해질 때. ⚠ 그때 VRAM 경합 계측 필수 (detector/llm 이
  이미 8/12GB — WSL 은 같은 GPU 공유).
- 구현 = [modules/motion/planner.py](../backend/modules/motion/planner.py):
  RRT-Connect (Kuffner & LaValle 2000) + shortcut smoothing, 순수 함수
  (collision_fn 주입 — fast 단위테스트 11개). 결정성 = 고정 seed 시퀀스
  (IK "복권 금지" 정책 동일). 직선 fast-path (자유공간 이동의 지배 케이스
  = 수십 회 검사로 끝) + max_checks 예산 (Pi 폭주 방지, 소진 = 명시 실패).

**B. wire — `Motion.Service.PLAN_PATH` (frontend 미노출, contract regen 불필요)**
- req: goal_joints / start_joints(None=현재) / floor_z / obstacle_points /
  gripper_open / **tcp_min_z** (운반 여유 — 경로 전 샘플 FK(tcp).z 하한.
  매달린 물체 바닥 여유의 **보수 근사**: 물체 자체는 충돌체 미모델 = v1
  한계 정직 표기. floor 평면을 올리는 방식은 어깨 링크가 상시 걸려 불가).
- res: found(**부정 데이터** — resolve index=-1 동형) / waypoints(**중간
  경유점만**, 직선=빈 리스트) / direct / planning_ms / checks (관측성 —
  Pi 성능 계측 1차 데이터).
- 실행 계약: 호출자가 waypoints 순차 MoveJ 후 goal MoveJ — 엣지 = 계획이
  검증한 관절 보간 그대로 (판정 경로 == 실행 경로).

**C. task 배선 — [steps/primitives.py](../backend/modules/tasks/pick_and_place/steps/primitives.py) `transit` (@step)**
- **계약: 계획 실패(부정 결과/wire 예외/timeout) = home 경유 폴백** — 옛
  실행 그대로라 동작 후퇴 없음. resolve 게이트 ④(path_from=home)는 이제
  "폴백 경로의 사전 증명"으로 역할 재정의 (게이트는 조기 타당성, 실행
  안전은 PLAN_PATH 의 실측 충돌검사). 플래너는 최적화지 의존성이 아니다.
- 교체 3곳: ① 관측 이동 (approach_observe — floor=검출 최저 base_z−여유)
  ② servo 진입 (pick — plan_pick 게이트와 같은 충돌 모델: floor_z +
  이웃 점군(ServoPlan.neighbors 신설) + 조 벌림) ③ **운반** (servo_pick
  `end_home=False` + execute_place 운반 transit — 쥔 채 최장 스윙 소멸.
  tcp_min_z = pick floor + 물체 높이 + 2cm(_CARRY_CLEARANCE_M ⚠ 실물 튜닝)).
- **유지**: task 종료 home (알려진 휴식 자세 = 다음 run 전제) / 실패·abort
  경로 (on_abort STOP 불변) / pick-only 종료 home.

**D. 검증 (sim 으로 증명된 것)**
| 항목 | 결과 |
|---|---|
| ruff / pyright | clean (전체) |
| fast loop | 420 passed (플래너 순수 11 + transit 폴백/운반 배선 5 신규) |
| sim (실 so101 URDF) | 벽 우회: 직선 막힘 전제 검증 → RRT 경로 발견 → **더 촘촘한 해상도로 독립 재검사 전 엣지 무충돌**. 자유 이동 = direct fast-path. PLAN_PATH e2e (현재 관절 시작) |
| PC 기준선 | 벽 우회 **436ms / 충돌검사 447회 / 경유점 3** — Pi 계측 비교점 (planning_ms/checks 가 매 계획 로그에 남음) |

**E. 실물에 남은 미지수 (정직)**
- **Pi(pi_hori1) 계획 시간 미계측** — PC 436ms 의 몇 배인지. plan_path 로그
  1줄이 첫 데이터. 느리면 그때 deployment 재배치(motion PC 이동)나 cuRobo.
- **운반 tcp_min_z 근사의 실효** — 물체 미모델이라 옆쪽 스침은 못 막음
  (바닥 여유만). 실물에서 운반 경로가 낮게 잡히면 _CARRY_CLEARANCE_M 튜닝
  또는 attached-box 모델링(다음 단계)으로.
- **직선/RRT 비율** — 실 workcell 에서 얼마나 자주 RRT 까지 가는지 (전부
  direct 면 플래너는 사실상 "검증된 직선"; 그것만으로도 home 왕복 제거 이득).
- 실물 CT before/after — 관측 전환·운반 구간이 체감 지점.
