...
- [contract_graph_viewer.md](docs/contract_graph_viewer.md) — **backend_v2 계약 토폴로지 그래프 뷰어 (2026-07-01 설계 완료, 구현 대기)**. module 계약(service/stream/event)을 Swagger flat 나열이 아닌 **노드+방향엣지 그래프**로. `backend_v2_modules.md §8` 의 "Developer contract viewer" 두 번째 소비자 구현. 결정: **runtime-served + frontend `/contract` 페이지(React Flow) + backend `GET /contract/graph` EXPORT (HTTP consume only)**. 이번 frontend gen 작업(`contract_snapshot`)이 토대 — attribution 살린 enrich 만 추가. 데이터 스키마 + 단계별 구현 + 경계 규칙 박제. "contract viewer" / "그래프" / "React Flow" / "토폴로지" 톤 나오면 본 문서.
...
- [backend_v2_status.md](docs/backend_v2_status.md) — backend_v2 진행 status + 다음 세션 handoff (현재 Motion D3 jog 완료, 123 test PASS, 다음 = C2 frontend 적응)
...
- [backend_v2.md](docs/backend_v2.md) — **framework 구현 spec (SSOT)**. 8 라운드 토론 (2026-06-25) 후 박힌 결정. 핵심 결정: **Zenoh 단일** (LocalTransport 박지 X — 측정 [backend/scripts/bench_transport.py](backend/scripts/bench_transport.py) 기반, 작은 message ~3us / 큰 ndarray 만 4%×N CPU 절감하지만 framework 두 갈래 비용 ↑), **Mirror[Bundle] 단일** (4 종 calibration sub-field 별도 Mirror 박지 X — atomic BA 단위 = CalibrationBundle), **exception propagation** (`{success, message, data}` envelope 박지 X), **Database-per-Module** (Storage Module 폐기 — centralization 이 Mirror 로 흡수, migration / dep 격리 자연 해결), **Module = plain class** (`@module` 데코 X, base class 강제 X), **robot-scoped vs robot-agnostic** (yaml `robots:` primary, constructor 의 robot_id 가 계약 검증 — "robot_id 받으면 robot-scoped" direction X, "robot-scoped 배치 시 robot_id 받아야" direction), **derived read model 패턴** (큰 payload decode dedup — CameraDecoded / DepthDecoded / TcpState / JointRad). spec = §2.7 scope + §3 4 primitive + §3.3.1 Mirror lifecycle + §3.6 Runtime lifecycle (instantiate→register→start) + §4 Owner/Reader 비대칭 + §11 Step 1-10. **새 세션 진입 시: backend_v2.md = SSOT. 기존 backend/ = 도메인 logic reference 만 (BA / Ruckig / IRLS / ChArUco / Step DSL / Open3D ICP 등 자산), framework 매몰 X.** 8 라운드 토론에서 박힌 자리 (Owner/Reader 비대칭 / CalibrationBundle 단일 / Mirror invariant 등) 의심하지 말고 따를 것 — 새 자세 박을 때 framework_dogfood_plan §13 history + backend_v2.md §14 anchor 다시 읽기. 사용자가 "framework 짜자" / "Step 1 시작" / "Mirror" / "Transport interface" 톤 던지면 본 문서. **2026-06-25 구현 진행** — Step 1 (Transport) + Step 2 (Contract) 완료 ([backend_v2/](backend_v2/), 21 test PASS, ruff/pyright clean). **다음 진입점 = Step 3 (Runtime + Module discovery)** — 자세히 backend_v2.md §15.
