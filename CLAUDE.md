...
**Hand-Eye capture-only flow (2026-06-23, 옛 Solver v3 폐기 후)** — backend = capture 수집 + storage append 만. BA / σ / observability / commit 다 **offline 분석 스크립트** ([`backend/scripts/calibrate_offline.py`](backend/scripts/calibrate_offline.py)) 로 분리. 핵심 철학: *대화형 BA 의 trauma 누적 차단 — capture 는 즉시, 분석은 사용자가 보며 천천히.* 옛 `handeye_ux_solver_v3_plan.md` / `bundle_adjust.py` / `observability_params.py` / `next_pose_planner.py` / `frozen_blocks` / `begin_refinement` / `compute` / `commit` 서비스 모두 폐기.
