...
| **D. 토픽 namespace rename** | ✅ **done** (commit `2270eba`) | `core/topic_map.py` + 모든 publish/subscribe caller | `horibot/{robot_id}/...` template + `BaseNode.r()` helper + `key_for()` (구 `topic_for`). 자세한 결정문은 [multi_robot_phase2_frontend.md §1](multi_robot_phase2_frontend.md) | topic_map.py 보면 끝 |
