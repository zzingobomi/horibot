...
| Calibration + Detector | 영속성 owner vs perception. Detector 는 Calibration Bundle 의 boot-time query Reader (intrinsic/hand_eye) |
...
### 3.1 Cross-module read (service call / boot query)

| Reader | 사용 패턴 | Source |
|---|---|---|
| Motion | **boot-time query** (`snapshot_bundle` 1회) | Calibration (link_offset/joint/sag → start() 시 kinematics build) |
...
| Reconstruction | call (1 회 build) + local kinematics 인스턴스 | Scan / Calibration (snapshot 매 build) |
| Task | call (step 별) | 모두 |
| Gamepad | publish stream | Motion (one-way) |
| Bridge | subscribe + relay | 모두 |

**현재 Mirror consumer = 0** (2026-07-02, §3.2). CalibrationBundle 이 boot-time configuration 으로 재분류 → Motion 은 boot-time `snapshot_bundle` query 로 읽음. 다른 Reader 는 매 호출 service call (per-request fresh). [backend_v2.md §4.4](backend_v2.md) robot-agnostic Reader 패턴.

### 3.2 데이터 성격 → primitive 분류 (Mirror 는 현재 deferred)

> **framework 차원 decision rule** — 새 module 설계 시 "이 데이터는 어느 칸인가?" 만 판단하면 primitive 가 결정된다.

| 데이터 성격 | 예시 | primitive |
|---|---|---|
| **Runtime telemetry** (지속 변화, observation) | TCP pose 20Hz, joint state 20Hz, motor state, force, temperature | **Stream** (continuous) 또는 **snapshot service** (point-in-time). Mirror ❌ |
| **Boot-time configuration** (프로세스 시작 시 확정, 이후 불변) | CalibrationBundle (5 kind), Capabilities | **Query** (boot 1회 조회). 변경은 다음 부팅부터. Mirror ❌ |
| **Control correctness state** (런타임 중 변경 → consumer 재계산 필요, 그러나 telemetry 는 아님) | **현재 도메인에 없음** | **Mirror** (해당 시). 지금은 consumer 0 |

**Mirror 의 status = deferred (정의 유지, 미사용)**:

> Mirror 는 framework 가 지원하는 primitive 로 **정의는 유지한다.** 그러나 현재 backend_v2 의 도메인에는 이를 사용하는 데이터가 없다. Runtime telemetry 는 Stream, boot-time configuration 은 Query 를 쓰기 때문에, "Control correctness state" 칸이 비어 있다. 향후 런타임 중 변경돼 consumer 재계산이 필요하면서 telemetry 도 config 도 아닌 상태가 실제 요구사항으로 등장하면 그때 Mirror 를 적용한다.
>
> **이건 "Mirror 추상화가 틀렸다" 가 아니라 "현재 consumer 가 0" 이라는 뜻.** CalibrationBundle 이 Mirror 의 유일한 consumer 였는데 (2026-07-01 spec) boot-time config 로 재분류되며 (2026-07-02, [calibration_module_boundary.md §6/§10.2](calibration_module_boundary.md)) 칸이 비었다. framework 에 primitive 가 있다는 이유로 억지로 데이터를 Mirror 로 노출하면 목적/수단 역전 (cargo-cult) 이라 하지 않는다.
...
| Owner | event | payload |
|---|---|---|
| Calibration | **CalibrationActivated** (bundle 변경 — "재시작 필요" 알림, Mirror trigger 아님) | `{robot_id, bundle_id, version}` |
...
**CalibrationActivated 의 성격** — bundle=config 라 **런타임 rebuild 트리거가 아니라 "재시작하면 새 bundle 적용됨" 알림** (frontend "재시작 필요" badge). `bundle_id` (DB row id) + `version` (monotonic) 은 "현재 active bundle vs 실행 중 프로세스가 로드한 bundle" 비교 (재시작 필요 여부 판단) + audit / debug 용. 자세히 [calibration_module_boundary.md §5](calibration_module_boundary.md).
...
| **Runtime state** | 현재 status | streaming on / torque enabled | 자주 변경 | status event / stream (frontend 가 bridge 로 구독). Mirror 아님 — §3.2 (현재 backend module 간 Mirror consumer 0) |
...
> **2026-07-01 갱신 — 두 소비자 모두 runtime-served 로 진화 (§8.1 원안 정정).**
> ① Frontend TS gen = 구현 완료 ([frontend_v2.md §2.1](frontend_v2.md)) — backend `/contract.json` EXPORT + frontend Node fetch. ② Developer contract viewer = 설계 완료·구현 대기 ([contract_graph_viewer.md](contract_graph_viewer.md)) — backend `/contract/graph` EXPORT + **frontend `/contract` 페이지(React Flow) 그래프**. **둘 다 build-time CLI 아니라 runtime-served** (아래 표의 `pnpm gen:types` / `python -m horibot.contract_viewer` 명령 열은 초기 스케치 — 실제 구현은 두 문서 참조). Swagger flat 나열이 아니라 노드+방향엣지 그래프인 이유 = 계약이 pub/sub 토폴로지라서 (contract_graph_viewer.md §0).
...
| 9 | **데이터 성격 → primitive**: telemetry = Stream/snapshot, boot-time config = Query, control-correctness state = Mirror. **Mirror 는 현재 consumer 0 → deferred** (CalibrationBundle 이 config 로 재분류, 2026-07-02). 정의는 유지, 미사용 | §3.2 |
...
| 14 | **CalibrationActivated event = "재시작 필요" 알림** (Mirror trigger 아님, bundle=config). `{robot_id, bundle_id, version}` 로 "active vs 로드된 bundle" 비교 → 재시작 필요 여부 판단 + audit | §3.4 |
...
```
                MotorDriver           CameraDriver      (leaf, dep 0)
                    ...
                    │                      │
                    ▼                      ▼
                 Motion ◄─ snapshot_bundle (boot query) ─ Calibration
                    │                                 │
            ┌───────┼──────────┬──────────────┬──────┘
            ▼       ▼          ▼              ▼
...
| **D** | **Motion** | A + Calibration 의 mock owner (snapshot_bundle service) | kinematics + IK + Ruckig + jog + TCP_SNAPSHOT service + **boot-time snapshot_bundle query 로 kinematics build** 검증 | mock motor 의 trajectory publish (move_l / jog) |
| **E** | **Calibration** | B + D | DB-per-Module 첫 검증 + ObjectStore 첫 검증 + capture loop + Bundle atomic + **boot-time config e2e** (offline commit → 재시작 → Motion 이 snapshot_bundle 로 kinematics build) + factory intrinsic pull seed idempotent | capture + Bundle commit + boot-query consumer 검증 |

**진행 (2026-07-02)**: **A~C + E ✅ 완료.** D(Motion) = jog(D3)까지 완료 — 단 **E 의 "Motion 이 snapshot_bundle 로 kinematics build" consumer 부분은 아직 미배선** (Motion.start() 가 calibration bundle 소비 안 함, [calibration_module_boundary.md §9](calibration_module_boundary.md)). E 는 persistence/capture/preview/boot + frontend + Playwright 4/4 검증 (mock+sim). 실 D405/offline BA 는 집.
...
| D | Calibration 의 **mock owner** | `tests/fixtures/mock_calibration_owner.py` (SNAPSHOT_BUNDLE service 만 — boot query 응답. ACTIVATED event 는 알림이라 D 검증 불필요) |
| E | (mock dep 없음 — B + D 의 실 Module 위. D 의 mock owner 제거) | — |

D 의 mock owner = Motion 의 boot-time snapshot_bundle query 검증 자리. 실 Calibration 박힌 자리 (E) 에선 mock owner 제거 + 실 boot-config e2e (offline commit → 재시작 → build).
...
- **Step D Motion 의 mock Calibration** — 실 Module X. `tests/fixtures/mock_calibration_owner.py` 같은 fixture / mock contract owner. Motion 의 boot-time `snapshot_bundle` query 검증 자리 (Mirror 아님)
