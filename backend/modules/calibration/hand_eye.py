...
class HandEyeCalibration:
    ...
    def validate(
        ...
        Bundle Adjustment 결과 같은 임의의 R/t로도 호출 가능 — Step 4 BA의
        검증 인터페이스가 동일.
        """
        if len(self.poses) < 2:
            return None
    ...
    def _compute_residuals(
        ...
        for p in self.poses:
            T_gripper2base = _make_T(p.R_gripper2base, p.t_gripper2base.reshape(3, 1))
            T_target2cam = _make_T(p.R_target2cam, p.t_target2cam.reshape(3, 1))
            T_target2base_list.append(T_gripper2base @ T_cam2gripper @ T_target2cam)
...
def _log_method_compare(compare: list[dict]) -> None:
    if not compare:
        return
    ref_name = next((c["method"] for c in compare if c.get("ref")), compare[0]["method"])
    logger.info("─── method 비교 (기준: %s) ───", ref_name)
    for c in compare:
        if c.get("ref"):
            logger.info("  %-12s  Δrot=  0.000°  Δt=  0.0mm  (기준)", c["method"])
        else:
            logger.info(
