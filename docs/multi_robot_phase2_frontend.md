...
### Mock backend
- [backend/nodes/device/motor_node_mock.py](../backend/nodes/device/motor_node_mock.py) — `MOTOR_CMD_JOINT` 받으면 internal position 즉시 갱신, `MOTOR_STATE_JOINT` 20Hz publish. 서비스는 success no-op.
- [backend/nodes/device/camera_node_mock.py](../backend/nodes/device/camera_node_mock.py) — 합성 JPEG 30Hz publish (MOCK CAMERA 라벨 + frame counter + 움직이는 dot). depth 미발행.
