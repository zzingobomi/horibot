...
- Kinematics: `Kinematics` Protocol + robot 별 adapter
...
- sag 적용 — §3.2 `SagCorrectedKinematics` (Decorator, fk 출력 / ik 입력 모두)
- link_offset 적용 — `PybulletKinematics` 생성자에서 patched URDF 로 로드 (이미 URDF 자체에 박힘, FK/IK 양쪽 자동)

**Protocol 정의:**

```python
# backend/modules/kinematics/kinematics.py
...
코드는 이 컨벤션을 가정해 하드코드:
- backend [pybullet_solver.py](../backend/modules/kinematics/adapters/pybullet_kinematics.py): `TCP_LINK_NAME = "tcp"`
- frontend [config.ts](../frontend/src/lib/robot/config.ts): `TCP_LINK_NAME = "tcp"`

새 robot type (so101_6dof 등) 통합 시 URDF 박을 때 `tcp` link 박아 둘 의무 — 빠뜨리면 `PybulletKinematics.__init__` 가 `tcp not found in URDF` raise (fail-fast).
...
|---|---|---|---|
| `PybulletKinematics(urdf_path)` | 현 [`Kinematics`](../backend/modules/kinematics/registry.py) refactor. DIRECT 모드 PyBullet. `calculateInverseKinematics` 가 DOF 자동 처리 | 5 / 6 / 임의 | Phase 1 |
| `MujocoIKSolver(mjcf_path)` | Track C 학습 env 와 single-model. mink (QP-IK) 또는 numerical IK | 미정 | Open Q 9 (도입 시점 미정) |

robots.yaml 의 `kinematics_backend: pybullet` 필드로 robot 별 결정. (mujoco 는 Track C 진입 시 추가 후보 — 현재 Literal 에서 제외)

**기존 코드와의 mapping:**

| 현재 ([solver.py](../backend/modules/kinematics/registry.py)) | 새 위치 |
|---|---|
| `Kinematics` 클래스 (process singleton + sag patch + link_offset patch) | `PybulletKinematics` (singleton 제거, robot 별 인스턴스, no sag patch — ideal 만) |
| `fk()` 안의 `apply_gravity_sag` 호출 | §3.2 `SagCorrectedKinematics.fk()` 로 이동 |
| `ik()` 안의 sag 역적용 | §3.2 `SagCorrectedKinematics.ik()` 로 이동 |
| `write_patched_urdf(URDF_PATH, link_offsets)` | `PybulletKinematics` 생성자 그대로 유지 (link_offset 은 URDF 자체에 박는 게 자연) |
| 클래스 상수 `URDF_PATH` (hardcoded) | 제거 — 생성자 인자로 받음 |

`RobotRegistry` 가 robot 별 `Kinematics` 인스턴스 보유: `registry.get_kinematics("omx_f_0") -> SagCorrectedKinematics(inner=PybulletKinematics(...), sag=..., joint_coords=...)`.

**Pydantic payload type:** Kinematics 는 process-internal API — 네트워크 boundary 아님 → Pydantic 모델 불필요. fk/ik 결과를 토픽으로 publish 하는 caller 가 자기 페이로드를 Pydantic 으로 wrap.

### 3.2 SagCorrectedKinematics (Decorator)

**역할**: 어느 inner `Kinematics` (PyBullet / MuJoCo) 든 동일하게 **sag 보정** 적용 + `Kinematics` Protocol 그대로 만족 (caller 입장에서 inner 와 동일 인터페이스). link_offset 은 inner solver 의 URDF patch 로 이미 처리되므로 여기서는 sag 만.

**Decorator 정의:**

```python
# backend/modules/kinematics/adapters/sag_corrected.py
from modules.kinematics.kinematics_backend import Kinematics
from core.sag_coordinates import SagCoordinates
import numpy as np

class SagCorrectedKinematics:
    """sag 보정을 inner Kinematics 의 fk 출력 / ik 입력에 적용. Kinematics Protocol 만족."""

    def __init__(self, inner: Kinematics, sag: SagCoordinates):
        self._inner = inner
        self._sag = sag
...
| 현재 ([solver.py](../backend/modules/kinematics/registry.py)) | 새 위치 |
|---|---|
| `apply_gravity_sag` 호출 in `Kinematics.fk` | `SagCorrectedKinematics.fk` |
| sag 역적용 in `Kinematics.ik` (반복 수렴) | `SagCorrectedKinematics.ik` (그대로 이동, 반복 수렴 로직 포함) |
| `SagCoordinates` 싱글톤 reference | 생성자 인자 `sag` 로 주입 (robot 별 인스턴스 가능) |

`RobotRegistry` 가 robot 별로 `SagCorrectedKinematics(PybulletKinematics(urdf_path), sag=SagCoordinates.for_robot(robot_id))` 생성 — caller (MotionExecutor / Detector / etc.) 는 항상 `SagCorrectedKinematics` 만 봄.
...
**Pydantic payload type:** MotorBackend 는 process-internal — Pydantic 불필요. `MotorNode` 가 backend wrap 후 토픽 (`<robot_id>/motor/state/joint`, `<robot_id>/motor/cmd/joint`) 의 페이로드를 `MotorJointState(StrictModel)` / `MotorCmd(StrictModel)` Pydantic model 로 정의 (§7.6). robot_id 는 topic key 의 `{robot_id}` placeholder 로 routing.
...
- `<robot_id>/camera/state/status` → `CameraStatus(StrictModel)` Pydantic
- `<robot_id>/camera/stream/raw` → binary JPEG (typed 안 함, §7.5)
- `<robot_id>/camera/stream/depth_frame` → binary framing + **JSON 헤더만** `DepthFrameHeader(StrictModel)` Pydantic (§7.5)

### 3.5 MotionExecutor (Protocol)

**역할**: motion 명령의 service handler 진입점. robot_id 받아서 해당 robot 의 `SagCorrectedKinematics` + `MotorBackend` + `TrajectoryRunner` 를 dispatch. caller 는 `BridgeClient` (frontend) 또는 Step DSL.
...
모든 method 의 request / response 는 Pydantic model (§7.6 `core.messages.motion`). robot_id 는 payload 에 박지 않고 topic key 의 `{robot_id}` placeholder 로 routing.

**Lifecycle / 스레딩:**

- Singleton (process 당 1개) — `RobotRegistry` 에서 모든 robot 의 dependency 받아 초기화
- thread-safe (motor backend / kinematics_backend 가 thread-safe 인 위에 직접 dispatch)
...
class MultiRobotMotionExecutor:
    ...
    def move_l(self, req: MoveLRequest) -> MoveResponse:
        solver = self._registry.get_kinematics(req.robot_id)        # SagCorrectedKinematics
...
| 직접 `get_default_kinematics()` 호출 | `registry.get_kinematics(robot_id)` |
...
| Instance | calibration 5종 (intrinsic, hand_eye, joint/link/sag offsets), instance 별 motor 설정 (USB port, baud), pointcloud scans, TSDF mesh | calibration ✓ / scans·mesh ✗ | `robot/instances/<robot_id>/` |
...
robots:
  omx_f_0:
    ...
    kinematics_backend: pybullet            # pybullet — get_kinematics() 분기 (mujoco 는 Track C 시점)
    camera_backend: realsense     # realsense | opencv — get_camera_capture() 분기 (mujoco 는 Track C 시점)
...
> robot_id reserved name (`system` / `task` / `instances` 등) 충돌 차단은 *defensive coding 명목 자리 N=1 사용자 명명 컨벤션상 실제 위험 0* 로 판단해 폐기 (2026-06-22). 미래 외부 협업자 yaml 입력 / N>1 도입 시점에 재도입 검토.
...
|---|---|
| `Kinematics` (Protocol) | per-robot 인스턴스 — `RobotRegistry` 가 robot_id → solver 매핑 |
...
| `robot/calibration/*.npz` (6종: intrinsic / hand_eye / handeye_poses / joint_offsets / link_offsets / sag_offsets) | `robot/instances/omx_f_0/calibration/` |
...
backend/modules/
├── kinematics/
│   ├── kinematics.py            (★ 신규 — Protocol + decorator)
│   ├── adapters/
...
위 도메인들은 *spec 차원의 예약* — RobotRegistry 의 robot_id validation 은 §4.4 결정대로 폐기 (2026-06-22). 도메인 자체는 토픽 라우팅에서 계속 사용 (`horibot/system/heartbeat` 등).
...
| Phase 1.B | **robot-scoped 코어 토픽** (`MOTOR_STATE_JOINT`, `MOTION_STATE_TCP`, `DETECTOR_STATE`, `TASK_STATE`) | multi-robot routing seam. topic key 의 `{robot_id}` placeholder 로 routing, payload 는 `StrictModel` 자손 |
...
| **모든 robot-scoped 상태 토픽** (`<robot_id>/.../state/*`) | typed | Pydantic BaseModel, `StrictModel` 상속 |
...
├── __init__.py
├── base.py                # StrictModel, EmptyData, ServiceRequest[T], ServiceResponse[T]
...
| 2 | Kinematics Protocol + PybulletKinematics + SagCorrectedKinematics Decorator (§3.1 / §3.2) | ✅ 완료 | `5bfbe72` |
| 3 | SagCorrectedKinematics Decorator (sag 분리) | ✅ 완료 (위와 같이) | `5bfbe72` |
...
1. **URDF 배치** (`robot/so101_6dof/urdf/`) + mesh STL — wrist 끝에 `tcp` link 박기 (§3.1 컨벤션, 안 박으면 Kinematics 부팅 시 fail-fast)
2. **FeetechBackend adapter 신규** (DynamixelBackend 와 같은 MotorBackend Protocol 만족)
3. **PybulletKinematics(so101_6dof.urdf)** 인스턴스 생성 (5DOF/6DOF 둘 다 호환 — Registry.get_kinematics 자동)
...
8. **Motion node 의 robot_id dispatch** (Phase 1 deferred) — service handler 가 payload 의 robot_id 보고 적절한 Kinematics/MotorBackend 라우팅. 또는 per-robot MotionNode 인스턴스 분리
...
- `docs/kinematics_solver_interface.md` — §3.1-3.2 의 Kinematics / SagCorrectedKinematics detail (본 문서로 흡수됨, 별도 분리 시 cross-ref)
