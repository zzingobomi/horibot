...
> ⚠️ **STATUS: deferred (2026-07-02) — 현재 consumer 0.** 데이터 성격 → primitive rule (anchor #2, §16.3): **Runtime telemetry → Stream/snapshot, Boot-time configuration → Query, Control-correctness state → Mirror.** 현재 도메인엔 "control-correctness state" 칸이 비어 있다 — telemetry (TCP/joint 20Hz) 는 Stream, calibration bundle 은 boot-time config 로 재분류되어 boot-query 를 쓴다. **따라서 아래 `CalibrationBundle` 예제는 실제 calibration wiring 이 아니라 Mirror 메커니즘 illustration (stand-in) 이다.** calibration 의 실제 계약은 [calibration_module_boundary.md §6](calibration_module_boundary.md) (boot-query, Mirror 안 씀) 참조. Mirror primitive 의 spec (startup ordering / value access / invalidate+refetch) 은 정의로 **유지** — 실제 control-correctness-state consumer 등장 시 이 spec 그대로 적용.
...
| 2 | ~~Mirror[CalibrationBundle] 단일~~ **→ SUPERSEDED (2026-07-02)**: CalibrationBundle = **boot-time configuration** (Mirror 아님, boot-query). Motion 은 start() 에서 `snapshot_bundle` 1회 조회. Mirror primitive 는 정의 유지하나 **현재 consumer 0 → deferred**. 근거: commit 은 backend 종료 상태 실행(RDB lock) → 런타임 중 bundle 변경 순간 없음, rollback 도 재시작. 데이터 성격 → primitive rule (Configuration vs Runtime State) = §3.3 상단 banner + §16.3, calibration 계약 = [calibration_module_boundary.md §6/§10.2](calibration_module_boundary.md). "atomic Bundle 단위" 자체는 유지 (query payload = atomic snapshot) | §3.3.3 + §4.1 |
...
| 9 | `Mirror(snapshot_service, snapshot_req, change_topic, value_cls, change_event_cls)` (method ref X) — **primitive 정의 유지, 현재 consumer 0 (deferred, anchor #2)** | §3.3 + §4.2 | 5 인자 모두 key + Pydantic class. `snapshot_req` = factory `Callable[[self], Req]`. cross-module method import 사라짐. 실제 control-correctness-state consumer 등장 시 적용 |
...
| **Boot-time configuration** (부팅 시 확정) | CalibrationBundle, Capabilities | **Query** (boot 1회). 변경은 다음 부팅부터. Mirror ❌ |
| **Control correctness state** (런타임 변경 → consumer 재계산) | **현재 도메인에 없음** | **Mirror** (해당 시). 현재 consumer 0 → deferred (§3.3) |
