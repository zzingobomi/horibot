...
>
> 🔌 **전체 배선도 = [plc_wiring.html](plc_wiring.html)** (브라우저로 열기 — 13가닥 전선 리스트 + 조립 순서 + 통전 전 체크). 구매 확정 제품(PC817 4CH 옵토보드 · HD-02 3.3V 릴레이) + **센서 12V 통일(SMPS 생략, §4 간단화 채택)** 기준. 릴레이가 3.3V 제품이라 아래 §3.4 의 "VCC ← 파이 5V" 는 **3.3V 로 대체**됨.
