...
OMX_F의 pick-and-place는 한 마디로 *프론트엔드의 자연어 prompt → LLM parse → 백엔드 step 시퀀스 → 모터 → 상태 환류*의 왕복. 다음 표를 머리에 박고 시작하자.

| # | 어디서 | 한 줄 요약 | 챕터 |
|---|--------|----------|------|
| 1 | Frontend | "Run" 버튼 → `bridge.callService(TASK_RUN, {task, prompt})` → WebSocket | §1 |
| 2 | Bridge   | WS message → Zenoh queryable 호출 → TaskNode | §1 |
| 3 | TaskNode | 자연어 prompt → **LLM (Qwen2.5-1.5B) parse** → (pick_object, place_object) | §3 |
| 4 | TaskNode | TASK_REGISTRY에서 task factory 호출 → `TaskRunner.run(task)` | §3 |
| 5 | TaskRunner | step 리스트를 한 개씩 `StepExecutor.execute(step)` | §4 §5 |
| 6 | StepExecutor | `_search_and_detect`: search pose 순회 + Grounding DINO + depth → 객체 위치 | §6 §6b |
| 7 | StepExecutor | `_grasp_policy` / `_place_policy`: detect 결과 → grasp_z / place_z 계산 | §7 |
| 8 | StepExecutor | `_gripper(verify_grasp=True)` + `VerifyGraspStep`: 잡힘 검증 | §5.4 |
| 9 | MotionNode + Solver | IK가 조인트각 5개를 풀어냄 (DLS) | §8 |
| 10 | TrajectoryRunner | Ruckig으로 50Hz 부드러운 명령 stream | §9 |
| 11 | MotorNode → Dynamixel | 모터 회전 (그리퍼는 부드러운 profile) | §10 |
| 12 | Backend → Frontend | `TASK_STATE`/`MOTION_STATE_TRAJ` 토픽으로 UI 갱신 | §2 |

§1과 §2가 *프론트엔드 양끝*, §3~§11이 *백엔드 본체*.

**핵심 분기점**:
- **§3 LLM parse**: 자연어 ("흰 큐브 들어서 파란 박스에 놔") → 영어 객체 이름 (`{"pick": "white cube", "place": "blue box"}`). GroundingDINO 가 한국어 약하니까 LLM 이 번역 + 분리 같이 처리.
- **§6b SearchAndDetect**: 객체가 한 view 에 다 안 잡힐 수 있어 `robot_poses.yaml` 의 `search_*` 자세 순회. 첫 detect 성공 시 break.
- **§7 GraspPolicy / PlacePolicy**: pick 은 *옆면 중간* (`base_z + height * 0.5`), place 는 *윗면 + drop_clearance(1cm)* (공중 떨구지 않게).
- **§5.4 verify_grasp**: close 직후 + lift 후 + place 직전 각각 `Present_Position` 검사 → 빈손이면 task fail (가짜 success 방지).

---

## 1. Frontend → Backend: "Run" 버튼이 누른 한 줄

`PromptPanel.tsx` ([frontend/src/components/panels/PromptPanel.tsx](frontend/src/components/panels/PromptPanel.tsx))에서 사용자가 자연어 prompt 박고 Run을 누르면:

```typescript
await run({ task: "pick_and_place", prompt: trimmed });
```

`prompt` 는 **자연어 한 줄** — "흰 큐브 들어서 파란 박스에 놔" 같이 한국어/영어 자유. 별도 pick/place 필드 X. backend 가 §3 에서 LLM 으로 split.
...
def _factory_pick_and_place(data: dict) -> Task:
    ...
    pick_object, place_object = parse_pick_place(prompt)   # ← LLM parse
...
frontend 가 보낸 *자연어 prompt 한 줄* 을 `parse_pick_place` 에 박아 `(pick, place)` 두 객체로 분리한 뒤 task factory 호출. 새 task 추가는 (1) factory 작성, (2) TASK_REGISTRY 등록.

### 3.1 LLM prompt parser — Qwen2.5-1.5B-Instruct 로컬 로드

[backend/modules/llm/prompt_parser.py](backend/modules/llm/prompt_parser.py):

- **모델**: `Qwen/Qwen2.5-1.5B-Instruct` (HuggingFace, ~3GB) — 한국어/영어 다국어, instruction-tuned, 작음.
- **로드 방식**: GroundingDINO 와 같은 패턴. `task_node.start()` 가 백그라운드 thread 로 `prompt_parser.preload()` 호출 → 첫 task 호출 시 이미 로드돼 있음. 첫 1회는 다운로드 + 로드 (~30초~1분), 이후엔 GPU inference 1-2초.
- **system prompt**: "객체 이름을 *영어로* 추출해라" — GroundingDINO 가 한국어 약하므로 LLM 이 *번역까지* 한 번에 처리.
- **출력**: JSON `{"pick": "white cube", "place": "blue box"}`. `place` null 이면 단순 pick (place 없음).

예시:
```
"흰 큐브 들어서 파란 박스에 놔"             → ("white cube", "blue box")
"pick the red ball and put it on the tray" → ("red ball", "tray")
"그냥 큐브 들어"                            → ("cube", None)
```

LLM 호출 실패 / 미로드 시 fallback — prompt 를 그대로 pick 으로 사용 (place=None). task 는 단순 pick 으로 동작.

---

## 4. Task = 선언형 step 리스트

`create_pick_and_place_task` ([pick_and_place.py](backend/modules/task/tasks/pick_and_place.py))를 보면 사실상 **데이터 구조** 하나를 반환할 뿐:
...
return Task(
    ...
    steps=[
        ...
        SearchAndDetectStep(prompt=pick_object, output_key="pick_pos", ...),
        SearchAndDetectStep(prompt=place_object, output_key="place_pos", ...),  # place_object 있을 때만
        GraspPolicyStep(input_key="pick_pos", output_key="grasp_xyz", ...),
        ...
        # ─── 아래는 place_object 있을 때만 ───
        PlacePolicyStep(input_key="place_pos", output_key="place_xyz", ...),
        MoveTCPStep(position_key="place_xyz", offset=(0,0,PLACE_HOVER_DZ), label="pre_place"),
        MoveTCPStep(position_key="place_xyz", offset=(0,0,0), label="place"),
        ...
        MoveTCPStep(position_key="place_xyz", offset=(0,0,PLACE_HOVER_DZ), label="post_place_retreat"),
        # ─── 공통 ───
...
**place_object 가 `None` 이면**: 위 list 의 *PlacePolicyStep ~ post_place_retreat* 7개가 빠진 단순 pick 형태로 생성. 큐브 잡고 lift 후 바로 home. release 안 함 → 잡은 큐브 떨어뜨리지 않음 (debug 용도).
...
- **출력 step (생산자)**:
  - `SearchAndDetectStep(output_key="pick_pos", ...)` — search pose 순회 + detect → context 에 *pick 객체 위치*
  - `SearchAndDetectStep(output_key="place_pos", ...)` — 같은 패턴으로 *place 객체 위치*
- **변환 step**:
  - `GraspPolicyStep(input_key="pick_pos", output_key="grasp_xyz", ...)` — pick 위치 → *옆면 그립 위치*
  - `PlacePolicyStep(input_key="place_pos", output_key="place_xyz", ...)` — place 위치 → *윗면 + 1cm release 위치*
- **입력 step (소비자)**: `MoveTCPStep(position_key="grasp_xyz" 또는 "place_xyz", offset=..., ...)` — 변환된 위치에서 꺼내 씀
- **검증 step**: `GripperStep(verify_grasp=True)`, `VerifyGraspStep` — 그리퍼 `Present_Position` 검사. 빈손이면 step fail → task fail.

이 *네 종류가 context dict를 통해 연결*되는 게 pick&place 의 데이터 흐름 골격. 두 객체를 따로 detect → 각자 정책 step 으로 가공 → MoveTCP 들이 같은 위치를 offset 만 변주하며 재사용 → 중간중간 검증으로 가짜 success 차단.
...
## 6b. SearchAndDetectStep — search pose 순회 + detect

§6 의 `GroundedDetectStep` 은 *현재 자세에서* 1회 detect. 근데 pick / place 객체가 한 view 에 다 안 잡힐 수 있음 — 큐브는 책상 위, 캐비넷은 책상 옆 같이 분리되면 카메라 한 자세에서 둘 다 못 봄.

`SearchAndDetectStep` 이 그 해법. `robot_poses.yaml` 의 `search_*` 자세 (lexical 정렬, e.g. `search_1`/`search_2`/`search_3`) 순회하면서:

```
for pose in search_poses:
    move_to(pose)
    sleep(0.5)
    res = grounded_detect(prompt)
    if res.success:
        context.set(output_key, res.position)
        return True  # 첫 성공 시 break
return False        # 모든 pose 에서 fail → step fail
```

내부적으로 `Service.MOTION_MOVE_J` + `Service.PERCEPTION_GROUNDED_DETECT` 둘 다 호출. 1회 detect 와 다르게 *어디선가 보이면 OK* 라 workspace 한계 안에서 robust.

**search pose 추가 방법**: `robot_poses.yaml` 에 `search_1: ...`, `search_2: ...` 박으면 `list_pose_names("search_")` 가 자동 수집. self-play 의 `_detect_with_recovery` 와 같은 패턴 (코드 위치는 다르지만 의도 동일).

코드: [step_executor.py](backend/modules/task/step_executor.py) `_search_and_detect`.
...
- 정책 핸들러: [step_executor.py](backend/modules/task/step_executor.py) `_grasp_policy`
- task에서 사용: [pick_and_place.py](backend/modules/task/tasks/pick_and_place.py)

---

## 7b. PlacePolicyStep — place 객체 위에 살짝 내려놓기

place_object 가 있을 때만 task list 에 들어가는 변환 step. *공중에서 큐브 떨구지 않게* place 위치를 결정.

`SearchAndDetect` 가 박은 `place_pos` (place 객체 윗면 base xyz) 를 받아:

```python
place_xyz = [x, y, top_z + drop_clearance]   # default drop_clearance = 0.010 (1cm)
```

place 객체 윗면에서 1cm 위 = release 시점 그리퍼 위치. 캐비넷 / 박스 / 트레이 같은 거 위에 큐브를 *살짝* 내려놓는 효과.

```
SearchAndDetect(place_object) → place_pos = (x, y, 캐비넷 윗면 z)
PlacePolicy(place_pos)         → place_xyz = (x, y, 캐비넷 윗면 + 1cm)
MoveTCP(place_xyz, +PLACE_HOVER_DZ) → pre_place (5cm 위 hover)
MoveTCP(place_xyz, 0)              → place
VerifyGrasp                        → 떨어지기 전 검증
GripperStep(open)                  → release
MoveTCP(place_xyz, +PLACE_HOVER_DZ) → post_place_retreat
```

**FK z 오차 영향**: detector 가 카메라(=TCP) 로 캐비넷 z 측정 + 그리퍼 z 도 같은 FK. 두 오차가 *상쇄*. `place_pos.z + 1cm` 명령하면 *실제로* 캐비넷 위 1cm 도달 (이론상).

- 정책 dataclass: [step_types.py](backend/modules/task/step_types.py) `PlacePolicyStep`
- 정책 핸들러: [step_executor.py](backend/modules/task/step_executor.py) `_place_policy`
...
pick-and-place 핵심 데이터 흐름 (place_object 있는 경우):

```python
SearchAndDetectStep(prompt=pick_object,  output_key="pick_pos")
# → context["pick_pos"]       = [x, y, top_z]
# → context["pick_pos_meta"]  = {"base_z", "height"}

SearchAndDetectStep(prompt=place_object, output_key="place_pos")
# → context["place_pos"]      = [x, y, top_z]
# → context["place_pos_meta"] = {"base_z", "height"}

GraspPolicyStep(input_key="pick_pos",  output_key="grasp_xyz")
# → context["grasp_xyz"]      = [x, y, base_z + height*0.5]

PlacePolicyStep(input_key="place_pos", output_key="place_xyz")
# → context["place_xyz"]      = [x, y, top_z + drop_clearance]

MoveTCPStep(position_key="grasp_xyz", offset=(0,0,0.06))   # pre_grasp
MoveTCPStep(position_key="grasp_xyz", offset=(0,0,0))      # grasp
MoveTCPStep(position_key="grasp_xyz", offset=(0,0,0.08))   # lift
MoveTCPStep(position_key="place_xyz", offset=(0,0,0.05))   # pre_place
MoveTCPStep(position_key="place_xyz", offset=(0,0,0))      # place
MoveTCPStep(position_key="place_xyz", offset=(0,0,0.05))   # post_place_retreat
```

키 종류:
- `pick_pos` / `place_pos` — raw detect 결과
- `*_meta` — 부수 정보 (base_z, height)
- `grasp_xyz` / `place_xyz` — 정책이 가공한 결과

이들이 *한 task 동안 살아 있는* `TaskContext` dict 안에 들어감. task 끝나면 GC.

### 11.1 왜 detect를 한 번만 하는가

1. **속도**: SearchAndDetect 는 *수 초~수십 초* (모델 추론 + pose 이동). 매 step 마다 다시 하면 비효율.
2. **일관성**: 한 번 결정한 위치를 끝까지 *같은 위치*로 유지.
...
```
[Frontend]
  PromptPanel "Run" 클릭 (자연어 prompt 한 줄)
...
[TaskNode 백그라운드 스레드]
    → parse_pick_place(prompt) → LLM (Qwen2.5-1.5B)             (§3.1)
        → {"pick": "white cube", "place": "blue box"}
    → factory(pick, place) → Task 객체
    → TaskRunner.run(task) → 새 스레드 _run_task                        (§5)
        for step in task.steps:
            → publish TASK_STATE
            → StepExecutor.execute(step)
                ┌────────────────────────────────────────────────────┐
                │ SearchAndDetectStep × N:                    (§6b)  │
                │   for pose in search_*:                            │
                │     MoveJ(pose) + grounded_detect(prompt)          │
                │     첫 성공 시 break                                │
                │   context.set("pick_pos" / "place_pos", ...)       │
                │                                                    │
                │ GraspPolicyStep:                              (§7) │
                │   grasp_z = base_z + height * 0.5  (옆면 중간)     │
                │                                                    │
                │ PlacePolicyStep:                              (§7b)│
                │   place_z = top_z + drop_clearance(1cm)            │
                │                                                    │
                │ MoveTCPStep × N:                                   │
                ...
                │         → publish MOTION_STATE_TRAJ                │
                │                                                    │
                │ GripperStep(verify_grasp=True) / VerifyGraspStep:  │
                │   close 후 / lift 후 / place 직전 Present_Position │
                │   ≥ 1900 인지 확인. 빈손이면 step fail → task fail │
                │                                                    │
                │ WaitStep / HomeStep                          (§5.4)│
                └────────────────────────────────────────────────────┘
...
- [ ] step 네 종류: 생산자 (SearchAndDetect) / 변환 (Grasp/PlacePolicy) / 소비자 (MoveTCP) / 검증 (verify_grasp/VerifyGrasp). context dict로 연결.
- [ ] frontend 는 *자연어 prompt 한 줄*만 박음. backend 가 LLM (Qwen2.5-1.5B) 으로 pick/place 영어 이름 추출.
- [ ] LLM 모델은 task_node.start() 에서 백그라운드 preload — GroundingDINO 와 같은 패턴.
- [ ] SearchAndDetect 는 `robot_poses.yaml` 의 `search_*` 자세 순회. 한 view 에 안 보이는 객체 robust 처리.
- [ ] GroundedDetect는 *color로 bbox* 찾고 *depth로 Z + height* 측정 — 두 신호 결합.
- [ ] depth stream은 *공용 자원* — 켜는 건 idempotent, 끄는 건 안 함.
- [ ] GraspPolicy 는 *항상 옆면 중간* (`base_z + height * 0.5`). PlacePolicy 는 *윗면 + 1cm* (공중 떨구지 않게).
- [ ] verify_grasp / VerifyGraspStep 은 그리퍼 `Present_Position` 검사. close 직후만 검증하면 lift/place 중 떨어진 케이스 못 잡으므로 lift 후 + place 직전 한 번 더.
