...
- §3 멀티로봇 UX — §2 에 흡수
- §4 store dict[robot_id] 화 — **다음 자리**. 본 Phase 2 의 mode sub-routes / mock backend / SSOT 정비 끝나고 실 검증 한 다음 진행.
...
## 다음 세션 — store dict[robot_id] 화

본 슬라이스의 마지막 reversible 자리. 이제 **observable conflict** 가 잡혀서 진행할 trigger 가 명확해짐.

### 지금 보이는 갭 (mock + multi-robot focus 진입 후)

- `/robots/omx_f_0/move` → `/robots/so101_0/move` 로 focus 전환 시 so101 이 **omx 의 motor state 자세를 그대로** 따라감. `robotStore.joints` 가 single-robot 으로 default robot 한 대만 받아서 — focus 가 바뀌어도 그 store 한 슬롯이 그대로 사용됨.
- so101 (현재 `enabled: false` viz-only) 도 sidebar 에서 navigate 가능하게 `capabilities: [move, calibrate]` 추가됨 — 시각화 검증 자리. dict 화 안 되면 자세 정확성은 미정.

### 영향 파일 (§4 결정문 line 287-303 참조)

- [frontend/src/store/](../frontend/src/store/) — `robotStore` / `cameraStore` / `motionStore` / `detectorStore` / `pointCloudStore` 5개 (global 은 그대로: `taskStore` / `sceneStore` / `systemStore` / `taskResultStore`)
- [frontend/src/hooks/useBridge.ts](../frontend/src/hooks/useBridge.ts) — 현재 `bridge.subscribe(Topic.MOTOR_STATE_JOINT, ...)` 한 번 호출 (BridgeClient 가 default robot_id 로 자동 expand). dict 화 시 `useRobots()` 로 enumerate → robot 별 subscribe loop.
- [frontend/src/api/bridge.ts](../frontend/src/api/bridge.ts) — `subscribe` / `callService` 가 robot_id 인자 받는 형태로 (현재는 default fallback).

### 시작 prompt 추천

> docs/multi_robot_phase2_frontend.md §4 결정문 + "다음 세션" 섹션 읽고, robotStore 부터 dict[robot_id] 로. useBridge 의 subscribe 도 robot enumerate. 검증은 `uv run python main.py --host mock` + frontend dev 서버, `/robots/omx_f_0/move` ↔ `/robots/so101_0/move` 전환 시 자세 따로 보이는지.

reversible 의 reversible — 만들어보고 발견. 한꺼번에 rebuild 말고 store 1개씩 점진 (§4 결정 3).
