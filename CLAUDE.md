...
- [frontend_v2.md](docs/frontend_v2.md) — **frontend 구현 spec (SSOT)**. backend_v2 의 4 primitive (Service/Stream/Event/Mirror) + Capability 를 React hook 1:1 노출. 13 anchor 결정 + Step F1-F5 + §12 testing 정책 (L4 = WebdriverIO, Playwright button hold 안 됨). 사용자가 "frontend" / "frontend_v2" / "useStream" / "useMirror" / "jog UI" 톤 던지면 본 문서.
- [frontend_v2_status.md](docs/frontend_v2_status.md) — frontend_v2 진행 status + 다음 세션 handoff (Step F1-F5 + L2 31 PASS + L4 Playwright 1 PASS + L4 WebdriverIO 1 PASS, 다음 = cleanup + backend bridge ws panic latent + Step E+ carry over)
