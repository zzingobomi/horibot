...
### 4.10 handover 4차 실패 근인 + 수취 전면 재설계 (2026-07-30 — "IK 전멸=종료" 폐기)

실패 런 = 07-29 밤 4건 (`20260728_231132` / `20260729_204308` / `223140` 등,
전부 `NoReachableGrasp`: 수취 자세 IK 54~64/64 전멸). **어제 "검출 오염" 1차
가설은 오진** — 마스크 픽셀 분석: 분홍(봉) 99%, 주축 span 85mm = 봉 전체가
깨끗이 보였다. 검출은 무혐의.

**근인 사슬** (재검출 점군 PCA + `full_diagnosis.py` 실측):

1. **봉이 omx 조 안에서 ~90° 회전 (물리 근인)** — 점군 주축 vs 계획축 이탈
   **84°(수평 제시) / 70°(hang)** 실측. 미믹 기어 약파지 + 픽→제시 재배향의
   중력 토크로 봉이 조 벌림축 둘레로 돌았다 (동반 지문: gap 93→10 / load
   −291→−32 = 파지 붕괴, advisory 라 통과). 수취는 **계획축 가정**으로
   자세족+겨냥점을 만들었으니 실물과 ~90° 어긋난 숙제 → 전멸. `seg_along`
   2mm(노출 46mm) 미스터리의 정체 = 점들이 가정축에 수직으로 뻗어 있었던 것.
2. **정확 자세 과잉 제약 (계획 근인)** — 옛 `_recv_orients` 는 "tool z ∥ ±봉축
   정확 일치"를 강제. 실측: 위치는 64/64 전부 도달(리치의 72% 지점), 죽는
   메커니즘은 **관절 한계에 걸리는 손목 방향 요구**. 잡기의 물리 제약은 "조
   닫힘축 ⊥ 봉축" 뿐 — 정확족 0~7해 지점에서 조건족은 10~17해.
3. **저품질 지대 제시 (배치 근인)** — 방향 자유도 히트맵(z0.18): 어제 수평
   E(x0.06,y−0.24)는 방향 도달 1~5% 최악 급, x0.25~0.35/y−0.30~−0.15 밴드가
   7~11% 최상. 수평 제시의 E 는 기하상 저품질 지대로 밀린다.
4. **실패 시점 관측 부재** — plan_receive 전멸 경로가 anchor/겨냥점을 trace 에
   안 남겨 "sim 생존 7 vs 실물 0" 갭을 확정 못 했다 (남은 후보 가설: pi_hori1
   에 eaik 미설치 → 수치 폴백. **부팅 로그 `IK=해석적(...)+polish` 1줄로 판정**
   — 수치라면 얇은 해집합을 못 찾는 게 설명된다. 단 PC sim 은 해석=수치 7/64
   동수라 solver 차이는 그 지점에선 무혐의).

**재설계** (전부 구현 + fast 473/sim feasibility 4 초록, 실물 미검증):

- **`measure_bar`** (신규 step): 재검출 점군 PCA 로 봉 축/자유단/파지점 실측
  ("눈을 믿는다" — 돌아간 봉은 돌아간 대로 잡는다). 품질 게이트(span
  ≥25mm/점군 ≥20) 미달 시 옛 FK 앵커 규약 폴백. 축 이탈 >25° = "조 안 회전"
  경고 + trace. sanity(파지점-TCP 거리가 봉 길이 밖 = 명시 실패).
- **`_grasp_orients`** (옛 `_recv_orients` 대체): 잡기 조건(조⊥축 정확)만
  강제, 남는 2 자유도(φ 조 방위 × ψ 접근 roll)를 8×8 격자 전개. ψ=0/180 이
  옛 족과 동일 (superset). `_receive_probe` 도 같은 족 (앞 12 × 여유 2단).
- **협상 루프** (`find_receive_shift` + `omx_nudge`, module 배선): plan_receive
  전멸 → so101 이 "봉이 δ 만큼 옮겨지면 잡는다"(±4cm 격자, 작은 이동 우선,
  스위트밴드 방향 동률 우선) → omx 가 자세 불변 평행이동 → 재관측→재실측→
  재계획 (상한 `_NEGOTIATE_MAX`=2). research 로는 capability-map 기반 handover
  pose optimization 계열의 런타임 축약.
- **trace 전량**: `measure_bar`(omx FK/joints/offset/축/이탈각/끝점/겨냥점),
  `plan_receive_dead`(겨냥점/축/그룹별 실패 사유/resolve message),
  `negotiate_shift`(탐색 격자), `omx_nudge`(δ/기각 사유). "로그가 없어 분석
  불가"는 이 클래스에서 재발 불가.
- 노브: `_RECV_SWEET_XY` (0.22,−0.235)→(0.26,−0.26) (히트맵 피크).

**재생 검증** (어제 실 점군 그대로, 실 URDF·캘): 수평 장면 = 실측축 84° 이탈
검출 → 그 지점 직접 해 0 → **협상 δ=(0,−20,0)mm 2번째 지점에서 생존**. hang
장면 = 70° 이탈 → **직접 8/256 생존** (어제 0/64). 어제 치명 장면 2개 모두 통과.

**오늘 밤 실물 체크리스트**: ① 미믹 기어 조치 (gap 93→10 붕괴는 SW 로 못 덮음
— 수취 접촉에서 낙하 위험) ② pi_hori1 부팅 로그 `IK=해석적` 확인 ③ 실패 시
trace `measure_bar`/`plan_receive_dead` 부터 (이제 전부 남는다) ④ 봉 회전이
실물에서 재현되면 `axis_dev_deg` 로 정량 확인.
