"""FkChain — robot kinematics model (Motion·Calibration 공용, 소유는 kinematics 도메인).

URDF chain 의 numpy FK — link_offset variable + gravity sag 보정 포함.

**PybulletKinematics 와 역할이 다름** (중복 아님):
- `PybulletKinematics` = 실행기 (joint → tcp, 고정 URDF, 빠름, 블랙박스). 런타임 제어.
- `FkChain` = 모델 (joint + zero offset + link error + gravity sag → tcp/torque, 파라미터
  가 변수로 노출돼 least_squares 로 미분 가능). offline BA 가 파라미터 추정에, Motion D4
  SagCorrected decorator 가 중력 토크 계산에 소비.

modules/motion 에 두는 이유: sag 는 캘리브레이션 알고리즘이 아니라 기구학 모델 →
kinematics 도메인(Motion) 소유. Calibration(offline BA 스크립트)은 소비자로 import.
파일이 여러 개(jacobian/gravity 등)로 늘면 modules/kinematics 로 추출 검토.
...
class FkChain:
    ...
    `link_offset` 은 arm joint 의 origin xyz / rotation 에 variable 로 박힌다.
    """
    ...
    def __init__(
        ...
        arm_joint_names: actuated arm joint names, motor id 순서. gripper 제외.
            motors.yaml 의 arm motor `.name` 으로 추출 (SSOT).
    ...
    def apply_gravity_sag(
        ...
        Args:
            ...
            sag_joint_indices: 0-indexed arm joint indices (sag 적용 자리).
                ...
                so101: physical.yaml 의 sag_joint_motor_ids. k_stiff 가 [0, 0] 이면 no-op.
