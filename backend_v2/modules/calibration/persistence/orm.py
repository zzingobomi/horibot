"""Calibration ORM — 4 entity. calibration 자기 `Base` (Database-per-Module).

실 DB(horibot.db) 스키마와 동일 — 옛 `backend/modules/calibration/orm.py` 에서
이월하되 공유 storage Base 대신 module-local Base, UtcDateTime 은 infra 승격분 사용.
"""
...
# calibration_* 테이블은 공유 Base 에 등록 (소유는 이 모듈, 마이그레이션은 루트 alembic).

# DB String ↔ Literal 경계 검증 (잘못된 값 유입 차단).
...
class CalibrationRunOrm(Base):
    ...
    __table_args__ = (
        # 같은 robot/kind 의 in_progress run 조회 가속 (partial index).
        Index(
...
class CalibrationResultOrm(Base):
    ...
    __table_args__ = (
        # invariant: 같은 (robot_id, kind) 는 active 하나만 (partial unique).
        Index(
...
class CalibrationCaptureArtifactOrm(Base):
    ...
    __table_args__ = (
        Index("idx_calibration_capture_artifacts_capture", "capture_id"),
        # 같은 capture 의 같은 kind artifact 는 하나만.
        UniqueConstraint("capture_id", "kind"),
    )


# ─── record ↔ ORM 양방향 mapper (boundary boundary) ───────────────


def run_record_to_orm(
    run: CalibrationRunRecord, *, force_status: str | None = None
) -> CalibrationRunOrm:
...
def result_record_to_orm(
    record: CalibrationResultRecord, *, run_id: int, is_active: bool
...
def orm_to_result(orm: CalibrationResultOrm) -> CalibrationResultRecord:
    # kind 로 알맞은 ResultRecord 선택 + result_data 검증 (discriminated union).
...
def capture_record_to_orm(
    capture: CalibrationCaptureRecord, *, run_id: int
) -> CalibrationCaptureOrm:
    # dict/list → JSON 문자열. JSON dict key 는 문자열화되므로 역변환 시 int() 복원.
...
def orm_to_capture(
    ...
    *,
    ...
    mp_raw = json.loads(orm.motor_positions) if orm.motor_positions else None
    motor_positions = (
        {int(k): int(v) for k, v in mp_raw.items()} if mp_raw is not None else None
    )
