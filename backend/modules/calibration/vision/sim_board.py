...
mock 카메라가 board_in_cam 포즈의 ChArUco 보드를 렌더 → CameraDecoded → board.detect
→ solvePnP → capture 파이프라인을 브라우저/실 하드웨어 없이 e2e 검증.

eye-in-hand 모델:
  T_cam_base   = FK(joints) @ X_cam2gripper
  board_in_cam = inv(T_cam_base) @ T_board_base
로봇이 움직이면 board_in_cam 이 일관되게 변함.

시뮬 전용 — 실 캘 경로(board.py)는 안 건드림. mock 카메라만 import (sim board 모드).
"""
...
# 시뮬 ground-truth — 실 SO-101 캘(2026-06-18) BA 결과 도출값.
...
# 실 SO-101 캘 자세 (joint degree) — FK-sim 에서 보드가 시야에 드는 검증된 자세.
...
_FRONTAL_PX = (700, 500)  # 7:5 (SQUARES_X:SQUARES_Y)
...
def render_charuco_at_pose(
    *,
    ...
    프론탈 보드 이미지를 board 3D 코너 투영 위치로 perspective warp. 보드가 카메라
    뒤 / 화면 밖이면 background 만.
    """
    out = np.empty((height, width, 3), dtype=np.uint8)
    out[:] = background

    corners3d = calib_board.board_corner_points_3d()  # (4,3) TL,TR,BR,BL
