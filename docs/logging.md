...
**각 Pi (hori1/2/3):** 현재 이미지는 **클래식 `ntpd`(`ntp` 패키지)** 로 동기 중 — 부팅 시
자동 시작(`enabled`)이라 **추가 셋업 불필요**. 상태 확인만 하면 됨:
```bash
systemctl status ntp | head -5   # "active (running)" + "enabled" 확인
ntpq -p                          # 맨 앞 '*' 붙은 소스 = 선택된 동기 서버, offset 열(ms)이 실측 오차
```
`*` 소스가 있고 offset 이 수 ms 이내면 정상 (실측: hori2 −1.3ms / hori3 −2.2ms, 2026-07-15).

> ⚠️ `timedatectl status` 는 `NTP service: n/a` 로 나오는데 **오동작 아님** — `timedatectl` 은
> systemd-timesyncd 만 인식하고 클래식 ntpd 는 모름. 실제 동기는 ntpd 가 하며 `System clock
> synchronized: yes` 가 이를 반영. **`sudo timedatectl set-ntp true` 를 치지 말 것** — timesyncd
> 는 ntp 패키지가 충돌 방지로 `masked` 해둔 상태라 무의미하고 혼란만 유발. chrony 도 설치 금지
> (ntpd 와 충돌). 검증은 위 `ntpq -p` 가 정본.

**Windows PC:** 관리자 PowerShell 에서:
```powershell
Set-Service w32time -StartupType Automatic
Start-Service w32time
w32tm /config /manualpeerlist:"time.windows.com,0x9 kr.pool.ntp.org,0x9" /syncfromflags:manual /update
Restart-Service w32time
w32tm /resync /force
w32tm /query /status   # "원본"에 NTP 서버(kr.pool.ntp.org 등)가 찍히면 OK
```

> ⚠️ **GUI "자동으로 시간 설정" 토글만으론 부족할 수 있음** — 실측(2026-07-15)에서 토글이 켜져
> 있는데도 `원본: Local CMOS Clock` (외부 NTP 미연결)로 표류 중이었음. 위 `w32tm /config` 로
> 서버를 명시해야 확실. 설정은 레지스트리+서비스에 영구 기록돼 **재부팅해도 유지**됨.
