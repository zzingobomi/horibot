...
- [task_imperative_framework.md](docs/task_imperative_framework.md) — **Task Framework 설계 확정 (2026-07-08, 구현 대기): Step/Slot DSL 폐기 → 명령형 async 함수 + FastAPI 철학 프레임워크**. §1 단일 원칙 = "개발자는 시나리오 의도만, 공통 관심사(바인딩/예외/취소/발행/게이트/로깅)는 프레임워크" + 기능 추가 기준 "반복 보일러플레이트를 제거하는가" (module framework `@service` 와 같은 수). `@task` signature=param SSOT→GET /tasks 스펙 자동 노출(§6 OpenAPI 등가), typed 도메인 예외 + "try 안 쓰는 게 기본"(§5), 도메인 어휘=ctx 밖 helper 함수로 자람(§3.3, ctx.pick 선제작 금지), FakeContext+CLI run_task.py 검증 경로(§7), stop in-flight 결함 해소, TREE→TRACE/PREVIEW 폐기. 구현 순서 6단계 = §9 (1–4 hardware 무관). frontend 는 이벤트 소비자(§8, 역방향 요구 금지). "task DSL" / "Step/Slot" / "@task" / "TaskContext" / "FakeContext" / "시나리오" 톤 나오면 본 문서 먼저 — backend_v2.md §17.4 의 "Slot 거의 그대로 포팅" 은 본 문서로 대체됨.
...
- [frontend_v2.md](docs/frontend_v2.md) — **frontend 구현 + status (SSOT, 단일 문서)**. backend_v2 의 4 primitive (Service/Stream/Event/Mirror) + Capability 를 React hook 1:1 노출. **지배 원칙 = 옛 `frontend/` 는 지우지 않은 의도적 carry-over reference — default 는 검증된 구조(dockview shell / RobotsLayout / 패널 registry / Sidebar 포함) 를 backend_v2 wire 로 포팅, defer 는 backend module 부재 feature 만 (anchor 1). "최소로 짓고 shell 나중에" 판단 금지.** 진행: jog vertical slice (F1-F5) 완료 — 31 vitest + 2 Playwright PASS (headed), lint 0 (데이터 경로 검증). **다음 foundation = Step F6 dockview shell carry-over** (jog 을 RobotMoveMode 패널셋으로 fold). **L4 e2e = Playwright headed 단일** (`headless:false` 필수 — 옛 "WebdriverIO 로 갈아탐 / Playwright button hold 못 함" 은 §15.2 에서 오진 정정: 진짜 원인은 headless 소프트웨어 렌더 메인스레드 starvation → jog rate↓, 입력 도구 무관). 13 anchor + §12 testing + §15 status. **계약 타입 생성(contract.ts) = §2.1** — backend `/contract.json` EXPORT + frontend Node `gen-contract.mjs` fetch (backend 접근 0), 노출 = `apps/contract_export.py::FRONTEND_EXPOSED` 한 곳 (2026-07-01 구현·검증 완료, 옛 frontend_contract_gen.md 통합·삭제). 사용자가 "frontend" / "frontend_v2" / "useStream" / "jog UI" / "shell" / "gen:types" / "contract 노출" / "openapi" 톤 던지면 본 문서. **주의: dockview/shell 관련 판단 전 옛 frontend shell 파일 Read 필수 — 2026-07-01 코드 안 읽고 "dockview=사치 defer" 단정한 실수 (§2.3).** (옛 frontend_v2_status.md 는 §15 로 통합·폐기.)
- [scene_contribution_architecture.md](docs/scene_contribution_architecture.md) — **frontend_v2 씬 시각화 소유권 모델 (구현 완료 2026-07-10)**. 1차 판정 = **"이 씬 객체를 누가 소유하나"**: Core chrome(씬) / **Scene object**(세계 — Robot/Camera/ScanMesh, 자기가 자기를 그림, 패널은 store 토글만) / **Feature overlay**(기능 — TaskResults topic 수명 + scenePart 패널 수명). 판별 질문 = "패널 닫으면 사라져야 하나?". scenePart = registry `scenePart:` 한 줄 + 패널 폴더 R3F 조각 (RobotProvider 대칭 — 안에서 `useRobotId()` 그대로), 기능/패널 기여로는 Scene.tsx diff 0. 좌표는 명시적 `<RobotFrame>`. scene 폴더 = objects/ overlays/ shared/ 3분할. **기각 확정 (§5, 재론 방지)** = descriptor DSL / runtime JSX 주입 / auto-wrap / "여러 패널이 원하면 layer"(증상 기반 — Camera frustum 사건) / 파생 pose DB 저장(waypoint tcp — revert됨, ghost 미리보기가 정답) / render-pass layer(defer). "패널에서 3D 표시" / "scenePart" / "씬 객체" / "frustum" / "ghost" 톤 나오면 본 문서.
- [robot_ownership_model.md](docs/robot_ownership_model.md) — **frontend workspace robot 소유권 아키텍처 규칙 (구현 완료 2026-07-08, frontend_v2, 미커밋)**. 불변식 = **robot은 패널이 소유** (Workspace/Route 아님, Route는 생성 시 초기값만), **바인딩은 오직 panel.robot** (환경=route/robot목록/개수 무관), **Add=항상 Spawn** (Workspace 공리, robot 무관), **Scene focus ⊥ Panel robot**. mechanism-free 아키텍처 문서 — RobotProvider/dockview params/updateParameters 등 구현체는 불변식 만족하는 한 자유. task 패널은 carve-out (task 바인딩). Open Q = layout 저장 범위(route별 vs 전역, 현행 route별 유지). UI 기능(패널 추가/삭제 헤더)은 [workspace_autohide_header.md](docs/workspace_autohide_header.md). "패널이 robot 어떻게 아나" / "route 밖 robot-scoped 패널" / "robot 셀렉터" / "ambient robot" 톤 나오면 본 문서.
- [backend_v2.md](docs/backend_v2.md) — **backend_v2 아키텍처 SSOT 단일 문서** (framework spec §1–§14 + **§16 Module catalog** + **§17 Task-first 운영 원칙/Task·PnP 설계** — 옛 backend_v2_modules.md / robot_agnostic_module_refactor.md / task_dsl_waypoint_port.md 를 2026-07-03 통합·폐기). 핵심 결정: **Zenoh 단일** (LocalTransport 박지 X — 측정 [backend/scripts/bench_transport.py](backend/scripts/bench_transport.py) 기반), **exception propagation** (`{success, message, data}` envelope 박지 X), **Database-per-Module** (Storage Module 폐기, 마이그레이션은 루트 단일 Alembic), **Module = plain class**, **Mirror deferred** (consumer 0 — telemetry=Stream/snapshot, boot-config=Query), **derived read model 패턴** (decode dedup), **robot-scoped 4 (motor/camera/camera_decoded/motion) 외 전부 robot-agnostic + robot_id 라우팅 = §2.7** (키의 robot_id=주소 / body 의 robot_id=req 필드, 파생 규칙, Bridge 자동주입 금지), **task-first** (§17 — 산출물=task 해내는 로봇, Day-1/Domain/Orchestration 3층 승격, 첫 task=단팔 PnP). anchor 22개 = §14. **새 세션 진입 시: backend_v2.md = SSOT, 진행 status = backend_v2_status.md. 기존 backend/ = 도메인 logic reference 만 (BA / Ruckig / IRLS / ChArUco / Step DSL / Open3D ICP 등 자산), framework 매몰 X.** 박힌 결정 의심하지 말고 따를 것 — 새 자세 박을 때 §14 anchor + framework_dogfood_plan §13 history 다시 읽기. 사용자가 "framework" / "Mirror" / "Module catalog" / "scope" / "robot_id 라우팅" / "Task DSL" / "PnP" 톤 던지면 본 문서.
- [framework_dogfood_plan.md](docs/framework_dogfood_plan.md) — **framework 도입 plan history**. 검토 phase 첫 큰 산출물. **2026-06-25 §15 Runtime-centric reframe** — §14 까지의 plan 이 "Node 가 최소 단위" 잘못된 전제 위에 있던 plan. 진짜 깨달음 = **Runtime (Process) 이 최소 단위, Module = 기능 묶음**. "Node 삭제" 의 진짜 의미 = 기능 제공자 (Module) 와 실행 컨테이너 (Runtime) 분리. backend_v2/ 폴더 (Phase 1 MVP + 7 test PASS) 2026-06-25 사용자 삭제 — §15 reframe 위에 다시 짬. Framework 핵심 책임 = "같은 코드가 어디 배치되든 그대로 동작" = distribution is runtime concern. 3 layer = Contract (`@service` / `@subscriber` / `@publishes`) → Runtime (registry + resolver + lifecycle + DI) → Transport (Local memory / Zenoh). key 형태 = path (`/storage/commit`). 4분류 (Handler/Cache/Worker/Adapter) 는 Module 의 유형 힌트 — framework 는 모름 (duck typing). 다음 step = Transport abstraction (`Transport(Protocol)` + ZenohTransport + LocalTransport). §15.8 = motion 영역 8 problem 으로 §15 reframe 코드 검증. §14 = history. §13 결정 17~20 (Node 잘못된 전제 / "Local 호출처럼" 표현 잘못 / `/` vs `.` / backend_v2 폐기). 사용자가 "Runtime" / "Module" / "Transport adapter" / "Contract layer" / "distribution is runtime concern" 톤 던지면 §15 진입.
...
- [jog_drift_tuning.md](docs/jog_drift_tuning.md) — SpeedTcp/SpeedJ cartesian jog 의 transient drift 진단 + fix 박제. **2026-06-17 update v2**: root cause 자리 _resolved-rate velocity-streaming_ 아키텍처 . SpeedJ/SpeedTcp 자리 폐기 + JogJ/JogTcp (LeRobot delta-pose) 자리 도입으로 directional transient 자체 사라짐 (수학적 보장).
...
**PnP 품질 gate (2026-06-12)** — `_srv_handeye_capture` 의 `cv2.solvePnP` 직후 reprojection error RMS 계산. `thresholds.HANDEYE_PNP_RMS_REJECT_PX=1.5px` 초과 시 capture **자동 reject** + 사용자 안내 ("이미지 품질 부족, 다시 시도해주세요"). ChArUco 코너 가림 / blur / 광량 부족 / board 미세 움직임이 만든 안 좋은 자세 자체를 _애초에 안 들임_ — trauma source 입구 차단.

**IRLS+Huber on `_physical_sag` (2026-06-12, [docs/handeye_robust_irls_plan.md §12](docs/handeye_robust_irls_plan.md))** — 운영 BA = [`bundle_adjust_hand_eye_physical_sag_irls`](backend/modules/calibration/bundle_adjust.py). 자세별 Huber weight `w_i = min(1, κ/r_i)`, κ=1.345·MAD/0.6745. outlier 자세의 X drift _수학적으로 bounded_. acceptance test (8장 + 합성 outlier 5°/20mm) — IRLS ΔR=baseline×0.55, w_outlier=0.118. 결과 dataclass `BundleAdjustPhysicalSagResult` 가 weights / outer_iter / cost_history / sigma_hat_history 통합 (non-IRLS BA 가 default 채움 호환). per-pose weight 가 `last_compute.per_pose_residual[i].weight` 로 frontend 전파 → [`PoseList`](frontend/src/components/panels/calibration/parts/PoseList.tsx) 가 color dot (정상/낮음/제외) 표시 — 수치 노출 X.
...
**Hand-Eye capture-only flow (2026-06-23, 옛 Solver v3 폐기 후)** — backend = capture 수집 + storage append 만. BA / σ / observability / commit 다 **offline 분석 스크립트** ([`backend/scripts/calibrate_offline.py`](backend/scripts/calibrate_offline.py)) 로 분리. 핵심 철학: _대화형 BA 의 trauma 누적 차단 — capture 는 즉시, 분석은 사용자가 보며 천천히._ 옛 `handeye_ux_solver_v3_plan.md` / `bundle_adjust.py` / `observability_params.py` / `next_pose_planner.py` / `frozen_blocks` / `begin_refinement` / `compute` / `commit` 서비스 모두 폐기.

새 service set (backend):

- `intrinsic/start | capture | save` — 옛 그대로.
- `handeye/start` — draft run 생성 (in_progress), intrinsic snapshot 박음.
- `handeye/capture` — depth_frame fresh + ChArUco PnP + RDB row + ObjectStore blob append. BA 호출 X.
- `handeye/undo_last_capture` — 마지막 capture row + blob 삭제.
- `handeye/finalize` — run status `in_progress → ready_for_analysis`. 이후 capture 막힘. **offline 분석 대기**.
- `handeye/reset | list_poses | preview_enable | thresholds`.

**Phase 1 Traffic Light** ([`capture_quality.py`](backend/modules/calibration/capture_quality.py)) — preview loop(5Hz) 가 detect + tilt + pose/rotation/translation diversity 로 🟢/🟡/🔴 판정. `CALIB_HANDEYE_PREVIEW` payload 의 `capture_verdict`/`capture_reasons`. **유일하게 살아남은 옛 piece** (Fisher / staged gating / Phase 1-2 / ghost preview 다 폐기).

**Offline 분석** — `calibrate_offline.py` 가 storage 에서 `ready_for_analysis` run 의 captures + blobs read → 5 stage BA (A/B/C/D/E) + sanity + LOOCV → `--commit` 으로 result rows insert + activate. backend storage_node 떠 있으면 RDB lock 충돌 — 종료 후 실행 권장. CLI: `uv run python scripts/calibrate_offline.py --robot <id> [--run-id N] [--stage E] [--commit]`.

**sag joint SSOT** — `robots.yaml::sag_joint_motor_ids` → `RobotConfig` → offline script 가 동일 path 로 SO-101 5축/6축 분기. OMX-named 하드코딩 3곳 제거 유지.

**캘 e2e 검증** — [`test_calibration_e2e.py`](backend/tests/test_calibration_e2e.py) (host_mock 단일 프로세스 + `CALIB_SIM_BOARD=1` ChArUco 시뮬 + 27 자세 capture + finalize → run status `ready_for_analysis` 검증) + [`test_calibration_distributed_sim.py`](backend/tests/test_calibration_distributed_sim.py) (3 프로세스 분산 `pc_sim + pi_motor_sim + pi_camera_sim` 같은 flow). offline BA 부분은 별도 unit test 후보 (현재 inline script 라 import 어려움).

**유저 플로우 (구현됨)**: ① [캘 시작] → 토크오프 손으로 자세 → overlay 실시간 🟢/🟡/🔴 → 🟢에서 [캡처] 반복. ② [세션 종료] → run `ready_for_analysis`. ③ backend 끄고 `calibrate_offline.py --commit` 실행 → DB active. ④ backend 재시작 → 적용 (SO-101 D405 = 공장 intrinsic 자동 seed, 내부캘 X).
...
- **Persisted timestamp 컨벤션: UTC-aware datetime** — ORM `DateTime(timezone=True)` / Pydantic Record `datetime` / wire ISO 8601 string / 생성 `datetime.now(UTC)`. float epoch 는 wire envelope / elapsed 측정 등 외부 boundary 자리에서만. 자세히는 [docs/storage_layer.md §5 Timestamp convention](docs/storage_layer.md)
