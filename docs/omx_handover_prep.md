...
1. **omx observe 포즈 — 계산 완료 (2026-07-23): 단일 포즈로 충분 가능성 높음.**
   ...
   - **남은 미지수 (실물)**: table_z 실제값(베이스가 책상 위 전제 — 다르면 포즈 시프트),
     ...
     (table_z 확정 시 재실행).
2. **analytic IK — 결정: 선제 작성 (2026-07-23 확정 → 같은 날 구현, 자리 실측
   정정).** ~~omx 는 EAIK 미적용(5축→수치 폴백)~~ → **실측 반증: EAIK 가 omx
   5축도 분해한다** (hasKnownDecomposition=True, FK-roundtrip 완전성 sim 통과 —
   test_motion_analytic_zyyyx). 그래서 최종 배치: 기본 = EAIK, 신규 작성한
   closed-form ([analytic_zyyyx.py](../backend/modules/motion/adapters/analytic_zyyyx.py))
   = **EAIK 부재 환경(pi_hori3 소스빌드 실패 클래스)의 결정성 백업**. J1=atan2
   겨냥 → 수직평면 2R → φ=θ2+θ3+θ4 / γ=θ5 (Ry·Rx 분해) 구조, snap/polish 패턴·
   부팅 로그 규약 계승. 두 경로 모두 같은 대조 테스트로 잠금.
...
## 9. 구현 기록 (2026-07-23 — sim 범위 완료, 실물 대기)

구현 순서 1·2·3·5·6·7·8 (= §7 "sim 으로 증명 가능" 전부) 완료. 4 의 observe
포즈는 **런타임 계산으로 승격** (오프라인 수치 하드코딩 대신 workcell centroid +
hand_eye 역산 + roll ψ 격자 resolve — table_z/캘 변경에 자동 추종),
`gripper_characterize.py` omx 실측만 실물 몫으로 남음.

| 산출물 | 자리 |
| --- | --- |
| mono z=0 역투영 + 왜곡 reproject | [projection.py](../backend/modules/detector/projection.py) `plane_points_from_pixels` / `project_base_to_pixel_distorted` |
| DETECT_PLANAR 서비스 (draft, 미노출) | [detector contract/module](../backend/modules/detector/contract.py) — depth 스냅샷 자체를 안 부름, 디버그 덤프 = depth 없는 raw 재현 세트(+plane_z/dist) |
| Z·YYY·X closed-form IK (EAIK 백업) | [analytic_zyyyx.py](../backend/modules/motion/adapters/analytic_zyyyx.py) — §8-2 실측 정정 참조 |
| CrossRobotChecker 정밀화 | [collision.py](../backend/modules/tasks/handover/collision.py) — URDF mimic 추종(미러 손가락 교차 버그 fix) + grip fraction + 판정별 margin (수취 12mm 노브) |
| 펜 기하 (끝점/파지점/노출 판정/랑데부) | [pen.py](../backend/modules/tasks/handover/pen.py) — 순수 numpy, frame 변환도 여기로 이동 |
| 시나리오 재배선 (A~E 전부) | [steps.py](../backend/modules/tasks/handover/steps.py) (노브 SSOT 블록 + `knob_snapshot()`) / [module.py](../backend/modules/tasks/handover/module.py) |
| run trace | [trace.py](../backend/modules/tasks/handover/trace.py) → `debug/handover/<ts>/{trace.jsonl, summary.json}` (summary 에 노브 스냅샷 각인) |
| omx workcell 앵커 | [instance.yaml](../robot/instances/omx_f_0/instance.yaml) `workcell:` (§8-1 도달 특성화 기반 — ⚠ 실물 보정) |

### 9.1 offline 도달성 특성화 (2026-07-23~24 — 흉터 5 를 sim 에서 선제로 밟음)

실 URDF/캘(repo DB)/base_pose 로 계획 기하를 offline probe 한 결과, **설계 두
곳이 실측으로 뒤집혀 재작업**됐다 (probe 스크립트 = scratchpad
handover_feasibility_probe / present_family_probe / so101_fine_yaw_scan /
handoff_clearance_probe — 결론은 전부 코드 주석과 회귀 테스트로 이식):

1. **제시 자세: "펜을 so101 로 조준" 은 도달 불가.** 펜 수평 유지 시 omx 5축
   다양체에서 임의 방위 펜은 tool-x 정하향/정상향 두 branch 뿐인데, 제시 높이
   (z 0.08~0.14)에선 J2–J4 ±90° 리밋으로 전멸 (0/60). **접선족** (펜 = TCP
   방위의 접선 — γ=±90° 특이족은 펜 축 둘레 spin 이 자유)은 60/60 도달 →
   [steps._present_orientations](../backend/modules/tasks/handover/steps.py):
   조준 A/B 선호 + 접선 spin 사다리 폴백. H(so101 겨냥점) = TCP + 노출방향
   ·오프셋 (펜이 so101 을 정조준하지 않아도 receive 는 재검출 기반이라 무관).
2. **so101 수취: v1 의 수평-우선 toward-상대 부채꼴은 전멸 (0/21).** so101 의
   공중 자세-고정 [pre,grasp] 도달은 극히 성김 — **포켓 실측: r≈0.23 · z 0.10 ·
   tilt 60~75° · 절대 yaw 225~270° (밴드 폭 30~45°)**. → 절대 yaw 15° 격자 ×
   tilt (60,75,45,90,30), 랑데부 정렬 = so101 sweet 반경 선호
   (_RENDEZVOUS_R_SO_M 0.27), 수취 관측도 (방위×고도×거리×roll) 사다리.
3. **근접 여유 실측: 채택 구성 쌍 링크 최근접 11.1mm** (so101 gripper_fixed ↔
   omx link5, 겨냥 tip-shift 1cm). margin 10mm 로 확정 — ⚠ 크로스캘 σ_t 8mm
   대비 얇다: **실물 1순위 확인 항목. 여유가 부족하면 더 긴 펜 사용** (노출 ↑
   = 여유 정비례). 14cm 펜 기준 계산이므로 첫 실물은 15cm+ 권장.
4. e2e mock 런 (mock driver + 실 DB 사본): boot→waypoint/workcell/hand_eye
   fail-fast→home→관측 자세 resolve(실 IK)→DETECT_PLANAR 실행까지 배선 검증
   (검출은 mock 합성 bbox 가 펜 기하 게이트에 걸려 명시 실패 — 정상 동작).

이 특성화 전부가 [test_handover_feasibility](../backend/tests/modules/test_handover_feasibility.py)
(sim) 로 잠김 — 노브/workcell 변경이 도달성을 죽이면 실물 전에 여기서 깨진다.

테스트: [test_detector_planar](../backend/tests/modules/test_detector_planar.py)
(왜곡 round-trip 회귀 포함) / [test_motion_analytic_zyyyx](../backend/tests/modules/test_motion_analytic_zyyyx.py)
(sim — 완전성/manifold/도달불가 확정, EAIK-off 백업 경로 픽스처) /
[test_handover](../backend/tests/modules/test_handover.py) (수취 순서 불변식 ·
짧은 펜/공중 재검출 실패 명시 · refine-miss coarse 진행 · preview 골격) /
[test_handover_collision](../backend/tests/modules/test_handover_collision.py)
(mimic/margin/grip plumbing) / [test_handover_feasibility](../backend/tests/modules/test_handover_feasibility.py)
(sim — §9.1). 게이트: ruff/pyright clean + full pytest (sim 포함) 초록.

**남은 것 = §7 실물 미지수 그대로** (전부 첫 런 특성화 항목): table_z 실측,
omx held 임계(gripper_characterize), thin+가림 공중 검출, URDF 조립 일치,
공통 워크스페이스 실측, 마찰/개구. 실행: `uv run --no-sync python
scripts/run_task.py srv/handover/run --param "pick_object=pen"` (mock),
pc.yaml handover 주석 해제는 실물 검증 후.
