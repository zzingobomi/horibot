...
class MockDetectorBackend:
    """합성 검출 — 중앙(high score) + 우상단(low score) 후보 2개 반환 (prompt 무관).

    실 모델 없이 DETECT 파이프라인(camera→adapter→Top-K→후보별 projection→base)을
    회사에서 검증. 후보 2개 = Top-K wiring + 후보 누적 + score desc 정렬 검증용.
    """
