"""봉(8×2×2cm 파란 각봉) handover 의 순수 기하 — 하드웨어/wire 0, 단위테스트 대상.
...
채택 설계 = **B/down 수직 제시** (scripts/handover_block_probe.py 결합 스윕,
2026-07-27 — 수평(접선)족은 so101 공중 수취 도달 0, 수직-아래 제시만 23개
전 게이트 통과):
  - omx 가 봉의 **한쪽 끝**을 top-down 으로 물고 (tool z ∥ 봉 축 = 노출 방향 u),
  - 제시는 봉을 **수직으로 세워 아래로 늘어뜨림** (tool z ↓) — 중력 모멘트 0
    (매달림 = 조 안 회전력 없음), 수직 봉은 방위 대칭이라 "so101 로 조준" 이
    필요 없다 (펜-era 조준/접선 문제와 검출 yaw 180° 모호성이 함께 소멸),
...
class BlockGrasp:
    ...
    ends: (파지점 xy, u) 두 후보 — u = 파지점→노출 끝(긴 자유부) XY 단위벡터.
      ...
      tool z ∥ u 규약 (pick 의 조 축 방위 + 제시 B/down 의 "노출부가 아래"가
      전부 이 벡터에서 파생).
    tcp_to_e_m: omx TCP(파지점) → so101 파지점 E 거리 (봉 축 방향) —
      제시(B/down)에서 E = TCP − (0,0,tcp_to_e).
...
def plan_block_grasp(
    ...
    *,
    ...
    grasp_frac: 잡는 끝에서 이 비율 지점을 문다 (모멘트 암↔조 접촉폭 트레이드
      오프 — B/down 제시는 매달림이라 모멘트 0, 접촉폭 확보가 우선).
