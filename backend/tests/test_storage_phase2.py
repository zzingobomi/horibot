...
def _make_session(robot_id: str = "test_robot", session_id: str = "s1") -> ScanSessionRecord:
    return ScanSessionRecord(
        robot_id=robot_id, session_id=session_id, created_at=_T0, label=None
    )
...
def test_scan_session_list_sorted_desc(repos: RepoBundle):
    timestamps = [_T0, _T0 + timedelta(seconds=200), _T0 + timedelta(seconds=100)]
    ...
    expected_desc = sorted(timestamps, reverse=True)
    assert [s.created_at for s in sessions] == expected_desc
